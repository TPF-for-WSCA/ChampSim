...
def get_instantiation_lines(cores, caches, ptws, pmem, vmem):
    ...
    upper_levels = {
        k: {"uppers": tuple(x[1] for x in v)}
        for k, v in itertools.groupby(
            sorted(upper_level_pairs, key=operator.itemgetter(0)),
    ...
    subdict_keys = (
        ...
        "wq_size",
        "_offset_bits",
        "_queue_check_full_addr",
    )
    upper_levels = util.chain(
        upper_levels,
        *({c["name"]: util.subdict(c, subdict_keys)} for c in caches),
        *(
            {p["name"]: util.chain(default_ptw_queue, util.subdict(p, subdict_keys))}
            for p in ptws
        ),
        {
            pmem["name"]: {
    ...
    yield pmem_fmtstr.format(
        _ulptr=vector_string(
            "&{}_to_{}_queues".format(ul, pmem["name"])
            for ul in upper_levels[pmem["name"]]["uppers"]
        ),
        **pmem
    )
    ...
    for ptw in ptws:
        yield "PageTableWalker {name}{{PageTableWalker::Builder{{champsim::defaults::default_ptw}}".format(
            **ptw
        )
        ...
        yield ".upper_levels({{{}}})".format(
            vector_string(
                "&{}_to_{}_queues".format(ul, ptw["name"])
                for ul in upper_levels[ptw["name"]]["uppers"]
            )
        )
        yield ".lower_level({})".format(
            "&{}_to_{}_queues".format(ptw["name"], ptw["lower_level"])
        )
    ...
    for elem in caches:
        yield "CACHE {}{{CACHE::Builder{{ {} }}".format(
            elem["name"], elem.get("_defaults", "")
        )
        ...
        yield from (
            v.format(**elem) for k, v in cache_builder_parts.items() if k in elem
        )
        yield from (
            v.format(**elem)
            for k, v in local_cache_builder_parts.items()
            if k[0] in elem and k[1] == elem[k[0]]
        )

        # Create prefetch activation masks
        if "prefetch_activate" in elem:
            yield ".prefetch_activate({})".format(
                ", ".join("access_type::" + t for t in elem["prefetch_activate"])
            )

        if elem.get("_replacement_data"):
            yield ".replacement<{}>()".format(
                " | ".join(
                    "CACHE::r{}".format(k["name"]) for k in elem["_replacement_data"]
                )
            )

        if elem.get("_prefetcher_data"):
            yield ".prefetcher<{}>()".format(
                " | ".join(
                    "CACHE::p{}".format(k["name"]) for k in elem["_prefetcher_data"]
                )
            )

        yield ".upper_levels({{{}}})".format(
            vector_string(
                "&{}_to_{}_queues".format(ul, elem["name"])
                for ul in upper_levels[elem["name"]]["uppers"]
            )
        )
        yield ".lower_level({})".format(
            "&{}_to_{}_queues".format(elem["name"], elem["lower_level"])
        )

        if "lower_translate" in elem:
            yield ".lower_translate({})".format(
                "&{}_to_{}_queues".format(elem["name"], elem["lower_translate"])
            )
    ...
    for cpu in cores:
        ...
        yield from (
            v.format(**cpu["DIB"]) for k, v in dib_builder_parts.items() if k in cpu
        )

        if cpu.get("_branch_predictor_data"):
            yield ".branch_predictor<{}>()".format(
                " | ".join(
                    "O3_CPU::b{}".format(k["name"])
                    for k in cpu["_branch_predictor_data"]
                )
            )
        if cpu.get("_btb_data"):
            yield ".btb<{}>()".format(
                " | ".join("O3_CPU::t{}".format(k["name"]) for k in cpu["_btb_data"])
            )
        ...
        yield ".realistic_perfect({})".format(cpu.get("realistic_perfect", 0))
        ...
        yield ".data_queues({})".format(
            "&{}_to_{}_queues".format(cpu["name"], cpu["L1D"])
        )
    ...
    yield "MEMORY_CONTROLLER& dram_view() override {{ return {}; }}".format(
        pmem["name"]
    )
    yield ""
    ...
    yield "    " + ", ".join(
        "{name}".format(**elem)
        for elem in itertools.chain(cores, ptws, caches, (pmem,))
    )
    yield "  };"
    yield "}"
    yield ""
