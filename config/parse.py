...
cache_deprecation_keys = {"max_read": "max_tag_check", "max_write": "max_fill"}
...
def executable_name(*config_list):
    name_by_parts = "_".join(
        ("champsim", *(c.get("name") for c in config_list if c.get("name") is not None))
    )
    name_by_specification = next(
        reversed(
            list(
                c.get("executable_name")
...
def duplicate_to_length(elements, n):
    ...
    return list(
        itertools.islice(
            itertools.chain(*(itertools.repeat(e, repeat_factor) for e in elements)), n
        )
    )
...
def normalize_config(config_file):
    ...
    cores = duplicate_to_length(
        config_file.get("ooo_cpu", [{}]), config_file.get("num_cores", 1)
    )
    ...
    cores = [
        util.chain(
            cpu,
            util.subdict(config_file, core_keys_to_copy),
            {"name": "cpu" + str(i), "_index": i},
        )
    ...
    caches = util.combine_named(
        ...
        (
            {"name": util.read_element_name(core, name), **core[name]}
            for core, name in itertools.product(cores, pinned_cache_names)
            if isinstance(core.get(name), dict)
        ),
        # Copy values from the config root, if these are dicts
        (
            {"name": util.read_element_name(core, name), **config_file[name]}
            for core, name in itertools.product(cores, pinned_cache_names)
            if isinstance(config_file.get(name), dict)
        ),
    ...
    ptws = util.combine_named(
        ...
        # Copy values from the core specification, if these are dicts
        (
            {"name": util.read_element_name(c, "PTW"), **c["PTW"]}
            for c in cores
            if isinstance(c.get("PTW"), dict)
        ),
        # Copy values from the config root, if these are dicts
        (
            {"name": util.read_element_name(c, "PTW"), **config_file["PTW"]}
            for c in cores
            if isinstance(config_file.get("PTW"), dict)
        ),
        # Apply defaults named after the cores
    ...
    cores = [
        util.chain(
            {n: util.read_element_name(cpu, n) for n in (*pinned_cache_names, "PTW")},
            cpu,
        )
        for cpu in cores
    ]
    ...
    return (
        ...
        caches,
        ptws,
        config_file.get("physical_memory", {}),
        config_file.get("virtual_memory", {}),
    )
...
def parse_normalized(
        ...
        caches = util.combine_named(
            ...
            itertools.islice(
                itertools.accumulate(
                    ...
                    lambda u, l: {
                        "name": l["name"],
                        "frequency": max(u.get("frequency", 0), l.get("frequency", 0)),
                    },
                ),
                1,
                None,
    ...
    ptws = util.combine_named(
        ...
        (
            {
                "name": cpu["PTW"],
                **defaults.ul_dependent_defaults(
                    *util.upper_levels_for(caches.values(), cpu["PTW"]),
                    queue_factor=16,
                    mshr_factor=5,
                    bandwidth_factor=2
                ),
    ...
    for cache, deprecations in itertools.product(
        caches.values(), cache_deprecation_keys.items()
    ):
        old, new = deprecations
        if old in cache:
            print(
                'WARNING: key "{}" in cache {} is deprecated. Use "{}" instead.'.format(
                    old, cache["name"], new
                )
            )
            cache[new] = cache[old]
    for ptw, deprecations in itertools.product(
        ptws.values(), ptw_deprecation_keys.items()
    ):
        old, new = deprecations
        if old in ptw:
            print(
                'WARNING: key "{}" in PTW {} is deprecated. Use "{}" instead.'.format(
                    old, ptw["name"], new
                )
            )
            ptw[new] = ptw[old]
    ...
    caches = filter_inaccessible(
        caches,
        [
            cpu[name]
            for cpu, name in itertools.product(cores, ("ITLB", "DTLB", "L1I", "L1D"))
        ],
    )
    ...
    for c in caches.values():
        c.setdefault(
            "prefetcher", "no_instr" if c.get("_is_instruction_cache") else "no"
        )
    ...
    tlb_path = itertools.chain.from_iterable(
        util.iter_system(caches, cpu[name])
        for cpu, name in itertools.product(cores, ("ITLB", "DTLB"))
    )
    l1d_path = itertools.chain.from_iterable(
        util.iter_system(caches, cpu[name])
        for cpu, name in itertools.product(cores, ("L1I", "L1D"))
    )
    caches = util.combine_named(
        ...
        (
            {
                "name": c["name"],
                "_offset_bits": "champsim::lg2(" + str(config_file["page_size"]) + ")",
            }
            for c in tlb_path
        ),
        (
            {
                "name": c["name"],
                "_offset_bits": "champsim::lg2(" + str(config_file["block_size"]) + ")",
            }
            for c in l1d_path
        ),
        caches.values(),
        ...
        (
            {
                "name": k,
                "_queue_check_full_addr": c.get("_first_level", False)
                or c.get("wq_check_full_addr", False),
            }
        ...
        (
            {
                "name": collections.deque(
                    util.iter_system(caches, cpu["L1I"]), maxlen=1
                )[0]["name"],
                "lower_level": "DRAM",
            }
            for cpu in cores
        ...
        (
            {
                "name": collections.deque(
                    util.iter_system(caches, cpu["L1D"]), maxlen=1
                )[0]["name"],
                "lower_level": "DRAM",
            }
            for cpu in cores
        ...
        (
            {
                "name": c["name"],
                "_replacement_data": [
                    replacement_context.find(f)
                    for f in util.wrap_list(c.get("replacement", []))
                ],
            }
            for c in caches.values()
        ),
        (
            {
                ...
                "_prefetcher_data": [
                    util.chain(
                        {
                            "_is_instruction_prefetcher": c.get(
                                "_is_instruction_cache", False
                            )
                        },
                        prefetcher_context.find(f),
                    )
                    for f in util.wrap_list(c.get("prefetcher", []))
                ],
            }
            for c in caches.values()
        ),
            ...
            (
                {
                    ...
                    "_branch_predictor_data": [
                        branch_context.find(f)
                        for f in util.wrap_list(c.get("branch_predictor", []))
                    ],
                }
                for c in cores
            ),
            (
                {
                    "name": c["name"],
                    "_btb_data": [
                        btb_context.find(f) for f in util.wrap_list(c.get("btb", []))
                    ],
                }
                for c in cores
            ),
    ...
    module_info = {
        "repl": util.combine_named(
            *(c["_replacement_data"] for c in caches.values()),
            replacement_context.find_all()
        ),
        "pref": util.combine_named(
            *(c["_prefetcher_data"] for c in caches.values()),
            prefetcher_context.find_all()
        ),
        "branch": util.combine_named(
            *(c["_branch_predictor_data"] for c in cores), branch_context.find_all()
        ),
        "btb": util.combine_named(
            *(c["_btb_data"] for c in cores), btb_context.find_all()
        ),
    }

    if compile_all_modules:
        modules_to_compile = [
            *set(itertools.chain(*(d.keys() for d in module_info.values())))
        ]
    else:
        modules_to_compile = [
            *set(
    ...
    return (
        ...
        modules_to_compile,
        module_info,
        util.subdict(config_file, extern_config_file_keys),
        util.subdict(config_file, env_vars),
    )
...
def parse_config(
    *configs,
    ...
    elements, modules_to_compile, module_info, config_file, env = parse_normalized(
        *normalize_config(merged_configs),
        merged_configs,
        branch_context=modules.ModuleSearchContext(
            [
                *(os.path.join(m, "branch") for m in module_dir),
                *branch_dir,
                os.path.join(champsim_root, "branch"),
            ]
        ),
        btb_context=modules.ModuleSearchContext(
            [
                *(os.path.join(m, "btb") for m in module_dir),
                *btb_dir,
                os.path.join(champsim_root, "btb"),
            ]
        ),
        replacement_context=modules.ModuleSearchContext(
            [
                *(os.path.join(m, "replacement") for m in module_dir),
                *repl_dir,
                os.path.join(champsim_root, "replacement"),
            ]
        ),
        prefetcher_context=modules.ModuleSearchContext(
            [
                *(os.path.join(m, "prefetcher") for m in module_dir),
                *pref_dir,
                os.path.join(champsim_root, "prefetcher"),
            ]
        ),
    ...
    module_info = {
        "repl": {
            k: util.chain(v, modules.get_repl_data(v["name"]))
            for k, v in module_info["repl"].items()
        },
        "pref": {
            k: util.chain(
                v, modules.get_pref_data(v["name"], v["_is_instruction_prefetcher"])
            )
            for k, v in module_info["pref"].items()
        },
        "branch": {
            k: util.chain(v, modules.get_branch_data(v["name"]))
            for k, v in module_info["branch"].items()
        },
        "btb": {
            k: util.chain(v, modules.get_btb_data(v["name"]))
            for k, v in module_info["btb"].items()
        },
    }
