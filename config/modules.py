...
def get_module_name(
    path, start=os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
):
...
class ModuleSearchContext:
    ...
    def data_from_path(self, path):
        return {
            ...
            "fname": path,
            "_is_instruction_prefetcher": path.endswith("_instr"),
        }
    ...
    def find_all(self):
        ...
        files = itertools.starmap(
            os.path.join,
            itertools.chain(*(zip(itertools.repeat(b), d) for b, d, _ in base_dirs)),
        )
...
def data_getter(prefix, module_name, funcs):
    return {
        ...
        "opts": {
            ...
            "CPPFLAGS": ("-DCHAMPSIM_MODULE",),
        },
        "func_map": {
            k: "_".join((prefix, module_name, k)) for k in funcs
...
def get_branch_data(module_name):
    return data_getter(
        "bpred",
        module_name,
        ("initialize_branch_predictor", "last_branch_result", "predict_branch"),
    )


def get_btb_data(module_name):
    return data_getter(
        "btb", module_name, ("initialize_btb", "update_btb", "btb_prediction", "btb_end_phase", "btb_invalidate_entry")
    )
...
def get_pref_data(module_name, is_instruction_cache=False):
    ...
    return util.chain(
        ...
        {
            "deprecated_func_map": {
                "l1i_prefetcher_initialize": "_".join(
                    (prefix, module_name, "prefetcher_initialize")
                ),
                "l1d_prefetcher_initialize": "_".join(
                    (prefix, module_name, "prefetcher_initialize")
                ),
                "l2c_prefetcher_initialize": "_".join(
                    (prefix, module_name, "prefetcher_initialize")
                ),
                "llc_prefetcher_initialize": "_".join(
                    (prefix, module_name, "prefetcher_initialize")
                ),
                "l1i_prefetcher_cache_operate": "_".join(
                    (prefix, module_name, "prefetcher_cache_operate")
                ),
                "l1d_prefetcher_operate": "_".join(
                    (prefix, module_name, "prefetcher_cache_operate")
                ),
                "l2c_prefetcher_operate": "_".join(
                    (prefix, module_name, "prefetcher_cache_operate")
                ),
                "llc_prefetcher_operate": "_".join(
                    (prefix, module_name, "prefetcher_cache_operate")
                ),
                "l1i_prefetcher_cache_fill": "_".join(
                    (prefix, module_name, "prefetcher_cache_fill")
                ),
                "l1d_prefetcher_cache_fill": "_".join(
                    (prefix, module_name, "prefetcher_cache_fill")
                ),
                "l2c_prefetcher_cache_fill": "_".join(
                    (prefix, module_name, "prefetcher_cache_fill")
                ),
                "llc_prefetcher_cache_fill": "_".join(
                    (prefix, module_name, "prefetcher_cache_fill")
                ),
                "l1i_prefetcher_cycle_operate": "_".join(
                    (prefix, module_name, "prefetcher_cycle_operate")
                ),
                "l1i_prefetcher_final_stats": "_".join(
                    (prefix, module_name, "prefetcher_final_stats")
                ),
                "l1d_prefetcher_final_stats": "_".join(
                    (prefix, module_name, "prefetcher_final_stats")
                ),
                "l2c_prefetcher_final_stats": "_".join(
                    (prefix, module_name, "prefetcher_final_stats")
                ),
                "llc_prefetcher_final_stats": "_".join(
                    (prefix, module_name, "prefetcher_final_stats")
                ),
                "l1i_prefetcher_branch_operate": "_".join(
                    (prefix, module_name, "prefetcher_branch_operate")
                ),
            }
        },
    )
...
def mangled_declarations(rtype, names, args, attrs=[]):
    ...
    yield from (
        "[[{}]] {} {}({});".format(attrstring, rtype, name, argstring) for name in names
    )


# Generate C++ code giving the mangled module specialization functions that are not implemented
def mangled_prohibited_definitions(
    fname, names, args=tuple(), rtype="void", *tail, attrs=[]
):
    ...
    yield from (
        '[[{}]] {} {}({}) {{ throw std::runtime_error("Not implemented"); }}'.format(
            attrstring, rtype, name, argstring
        )
        for name in names
    )
...
def discriminator_function_declaration(
    fname, rtype, args, varname, secondary_varname, classname
):
    yield "template <unsigned long long {}, unsigned long long {}>".format(
        *sorted([varname, secondary_varname])
    )
...
def discriminator_function_definition_void(
    fname, args, varname, zipped_keys_and_funcs, classname
):
    # Discriminate between the module variants
    yield from (
        "  if constexpr (({} & {}::{}) != 0) intern_->{}({});".format(
            varname, classname, k, n, ", ".join(a[1] for a in args)
        )
        for k, n in zipped_keys_and_funcs
    )


# Generate C++ code for the body of a discriminator function that returns nonvoid
def discriminator_function_definition_nonvoid(
    fname, rtype, join_op, args, varname, zipped_keys_and_funcs, classname
):
    # Declare result
    yield "  " + rtype + " result{};"
    ...
    # Discriminate between the module variants
    yield from (
        "  if constexpr (({} & {}::{}) != 0) result = joiner(result, intern_->{}({}));".format(
            varname, classname, k, n, ", ".join(a[1] for a in args)
        )
        for k, n in zipped_keys_and_funcs
    )
...
def discriminator_function_definition(
    fname, rtype, join_op, args, varname, zipped_keys_and_funcs, classname
):
    yield "{"

    if rtype == "void":
        yield from discriminator_function_definition_void(
            fname, args, varname, zipped_keys_and_funcs, classname
        )
    else:
        yield from discriminator_function_definition_nonvoid(
            fname, rtype, join_op, args, varname, zipped_keys_and_funcs, classname
        )
...
def get_module_variant_declarations(
    fname, fnamelist, args=tuple(), rtype="void", *tail, attrs=[]
):
...
def get_discriminator(
    ...
    *tail,
    ...
    yield from discriminator_function_declaration(
        fname, rtype, args, varname, secondary_varname, classname
    )
    yield from discriminator_function_definition(
        fname,
        rtype,
        join_op,
        args,
...
def constants_for_modules(prefix, mod_data):
    yield from (
        "constexpr static unsigned long long {0}{2:{prec}} = 1ull << {1};".format(
            prefix, n, data["name"], prec=max(len(k["name"]) for k in mod_data)
        )
        for n, data in enumerate(mod_data)
    )
...
def get_ooo_cpu_module_lines(branch_data, btb_data):
    ...
    btb_variant_data = [
        ...
        (
            "btb_prediction",
            (("uint64_t", "ip"),),
            "std::tuple<uint64_t, uint64_t, uint8_t, uint64_t>",
        ...
        itertools.chain(
            ...
            *(
                get_module_variant_declarations(
                    fname, [v["func_map"][fname] for v in branch_data.values()], *finfo
                )
                for fname, *finfo in branch_variant_data
            ),
            *(
                get_module_variant_declarations(
                    fname, [v["func_map"][fname] for v in btb_data.values()], *finfo
                )
                for fname, *finfo in btb_variant_data
            )
        ),
        itertools.chain(
            *(
                get_discriminator(
                    ...
                    btb_varname,
                    [
                        (branch_prefix + v["name"], v["func_map"][fname])
                        for v in branch_data.values()
                    ],
                    *finfo,
                    classname=classname
                )
            ...
            *(
                get_discriminator(
                    ...
                    branch_varname,
                    [
                        (btb_prefix + v["name"], v["func_map"][fname])
                        for v in btb_data.values()
                    ],
                    *finfo,
                    classname=classname
                )
...
def get_cache_module_lines(pref_data, repl_data):
    ...
    return (
        itertools.chain(
            ...
            *(
                get_module_variant_declarations(
                    fname, [v["func_map"][fname] for v in pref_data.values()], *finfo
                )
                for fname, *finfo in pref_nonbranch_variant_data
            ),
            ...
            *(
                mangled_prohibited_definitions(
                    ...
                    [
                        v["func_map"][fname]
                        for v in pref_data.values()
                        if not v.get("_is_instruction_prefetcher")
                    ],
                    *finfo
                )
                for fname, *finfo in pref_branch_variant_data
            ),
            *(
                get_module_variant_declarations(
                    fname,
                    [
                        v["func_map"][fname]
                        for v in pref_data.values()
                        if v.get("_is_instruction_prefetcher")
                    ],
                    *finfo
                )
                for fname, *finfo in pref_branch_variant_data
            ...
            *(
                get_module_variant_declarations(
                    fname, [v["func_map"][fname] for v in repl_data.values()], *finfo
                )
                for fname, *finfo in repl_variant_data
            )
        ),
        itertools.chain(
            *(
                get_discriminator(
                    ...
                    repl_varname,
                    [
                        (pref_prefix + v["name"], v["func_map"][fname])
                        for v in pref_data.values()
                    ],
                    *finfo,
                    classname=classname
                )
                for fname, *finfo in itertools.chain(
                    pref_nonbranch_variant_data, pref_branch_variant_data
                )
            ),
            *(
                get_discriminator(
                    ...
                    pref_varname,
                    [
                        (repl_prefix + v["name"], v["func_map"][fname])
                        for v in repl_data.values()
                    ],
                    *finfo,
                    classname=classname
                )
