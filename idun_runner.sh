...
do
    ...
    do
        ...
        # CVP-1
        srun --account=share-ie-idi -J num-collection-$(basename ${bin}) --mail-user=romankb@ntnu.no --mail-type=FAIL --mem-per-cpu=${mem_per_cpu} -n1 -c${max_core_count} -t$timelimit -o /cluster/work/romankb/latency-cvp1-${dir}-$(basename ${bin})-%j.out  -e /cluster/work/romankb/latency-cvp1-${dir}-$(basename ${bin})-%j.err  python ~/ChampSim/data_collector.py --warmup ${warmup} --evaluation ${simulation} --experiment_executable ${bin} --traces_directory /cluster/work/romankb/dataset/cvp1_secret/srv_part1 --nosub --output_dir /cluster/work/romankb/results/${dir}_${count}${suffix:+_$suffix}/cvp1_server/sizes_$(basename ${bin})/  &>> /cluster/work/romankb/pyrunner_latency_fixed_ipc1_server_$(basename ${bin}).log &
