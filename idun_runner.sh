...
suffix="basic_block_count"
binary_dir=("region_sensitivity") # "btb_size_region_sensitivity" "btb_4k_10b_tag_sensitivity" "btb_4k_12b_tag_sensitivity" )
count=20
timelimit="8:00:00"
warmup=50000000
simulation=50000000
...
do
    ...
    do
        echo $bin
        # CVP-1 secret SRV
        # srun --account=share-ie-idi -J num-collection-$(basename ${bin}) --mail-user=romankb@ntnu.no --mail-type=FAIL --mem-per-cpu=${mem_per_cpu} -n1 -c${max_core_count} -t$timelimit -o /cluster/work/romankb/latency-cvp1-${dir}-$(basename ${bin})-%j.out    -e /cluster/work/romankb/latency-cvp1-${dir}-$(basename ${bin})-%j.err    python ~/ChampSim/data_collector.py --warmup ${warmup} --evaluation ${simulation} --experiment_executable ${bin} --traces_directory /cluster/work/romankb/dataset/cvp1_secret/srv_part1   --nosub --output_dir /cluster/work/romankb/results/${dir}_${count}${suffix:+_$suffix}/cvp1_srv_secret/sizes_$(basename ${bin})/              &>> /cluster/work/romankb/pyrunner_latency_fixed_cvp1_srv_$(basename ${bin}).log &
        # IPC-1 Benchmarks
