...
uint64_t PageBTB_writes = 0, PageBTB_readsBeforeWrite = 0, RegionBTB_readsBeforeWrite = 0, RegionBTB_writes = 0, PageBTB_reads = 0, RegionBTB_reads = 0;

uint64_t _isa_shiftamount = 2;
uint64_t _BTB_SET_BITS = 0;
uint64_t _FULL_TAG_MASK = 0;
...
BASIC_BTB_ENTRY *basic_btb_find_entry(uint8_t cpu, uint64_t ip) {
  ...
  for (uint32_t i = 0; i < BASIC_BTB1_WAYS; i++) {
    if (get_tag(basic_btb1[cpu][set][i].ip_tag) == get_tag(ip)) {
...
std::tuple<uint64_t, uint64_t, uint8_t, uint8_t> O3_CPU::btb_prediction(uint64_t ip) {
  ...
  uint8_t branch_type = btb_entry->branch_type;
  ...
  if (branch_type == BRANCH_RETURN) {
    ...
    return {target, btb_entry->ip_tag, true, BRANCH_RETURN};
//    return std::make_pair(target, always_taken);
    ...
    return std::make_pair(basic_btb_indirect[cpu][basic_btb_indirect_hash(cpu, ip)], always_taken);
	  ...
	  return {target, btb_entry->ip_tag, branch_type != BRANCH_CONDITIONAL, branch_type};
