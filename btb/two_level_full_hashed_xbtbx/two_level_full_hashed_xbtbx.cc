...
struct BTBEntry {
  ...
  auto tag() const
  {
    ...
    // _BTB_TAG_SIZE
    if (_BTB_TAG_SIZE> 0){
      const uint64_t tag_mask = (1 << _BTB_TAG_SIZE)-1;
      while(addr != 0)
      {
        tag = tag ^ (addr & tag_mask);
...
#elif USE_SRRIP
std::map<O3_CPU*, champsim::msl::srrip_table<BTBEntry>> L2_BTB;
...
#endif
std::map<O3_CPU*, champsim::msl::lru_table<BTBEntry>> L2_BTB;
...
std::tuple<uint64_t, uint64_t, uint8_t, uint8_t> O3_CPU::btb_prediction(uint64_t ip)
{
  ...
  if (_BTB_TAG_REGIONS && !filter_hit.has_value()) {
    ...
    if (small_way_regions_enabled && region_idx_.has_value()) {
      full_small = ::L2_BTB.at(this).check_hit({ip, 0, branch_info::ALWAYS_TAKEN, region_idx_.value(), 0});
      ...
      partial_small = ::L2_BTB.at(this).check_hit({ip, 0, branch_info::ALWAYS_TAKEN, std::tuple<uint16_t, uint16_t, uint64_t>{(uint16_t)-1, 0, 0}, 0});
    }
    if (big_way_regions_enabled && region_idx_.has_value()) {
      full_big = ::L2_BTB.at(this).check_hit({ip, 0, branch_info::ALWAYS_TAKEN, region_idx_.value(), BTB_TARGET_SIZES.end()[-2]});
      if (full_big.has_value() && !utilise_regions(full_big.value().target_size)) {
        full_small = std::nullopt;
      }
    } else if (!big_way_regions_enabled) {
      partial_big = ::L2_BTB.at(this).check_hit(
          {ip, 0, branch_info::ALWAYS_TAKEN, std::tuple<uint16_t, uint16_t, uint64_t>{(uint16_t)-1, 0, 0}, BTB_TARGET_SIZES.end()[-2]});
    }
    full_64 = ::L2_BTB.at(this).check_hit({ip, 0, branch_info::ALWAYS_TAKEN, std::tuple<uint16_t, uint16_t, uint64_t>{(uint16_t)-1, 0, 0}, 64});
    ...
    btb_entry = ::L2_BTB.at(this).check_hit({ip, 0, ::branch_info::ALWAYS_TAKEN, std::tuple<uint16_t, uint16_t, uint64_t>{0, 0, 0}, 0});
  } else {
...
void O3_CPU::update_btb(uint64_t ip, uint64_t branch_target, uint8_t taken, uint8_t branch_type)
{
  ...
  if (branch_target != 0) {
    ...
    l1evicted = ::L1_BTB.at(this).fill(L1opt_entry.value_or(fill_entry), 0);
  }
  ...
  if (small_way_regions_enabled && tmp_region_idx.has_value()) {
    small_hit = ::L2_BTB.at(this).check_hit({ip, 0, type, tmp_region_idx.value(), 0});
    if (small_hit.has_value() && !utilise_regions(small_hit.value().target_size)) {
      small_hit = std::nullopt;
    }
  } else if (!small_way_regions_enabled) {
    small_hit = ::L2_BTB.at(this).check_hit({ip, 0, type, std::tuple<uint16_t, uint16_t, uint64_t>{(uint16_t)-1, 0, 0}, 0});
  }
  if (big_way_regions_enabled && tmp_region_idx.has_value()) {
    big_hit = ::L2_BTB.at(this).check_hit({ip, 0, type, tmp_region_idx.value(), BTB_TARGET_SIZES.end()[-2]});
    if (big_hit.has_value() && !utilise_regions(big_hit.value().target_size)) {
      big_hit = std::nullopt;
    }
  } else if (!big_way_regions_enabled) {
    big_hit = ::L2_BTB.at(this).check_hit({ip, 0, type, std::tuple<uint16_t, uint16_t, uint64_t>{(uint16_t)-1, 0, 0}, BTB_TARGET_SIZES.end()[-2]});
  }
  hit_64 = ::L2_BTB.at(this).check_hit({ip, 0, type, std::tuple<uint16_t, uint16_t, uint64_t>{(uint16_t)-1, 0, 0}, 64});
  ...
  if (!small_hit.has_value() && !big_hit.has_value() && !hit_64.has_value()) {
    lru_elem = ::L2_BTB.at(this).get_replacement_element(::BTBEntry{ip, 0}, num_bits);
  }
  ...
  if (require_region) {
    ...
    if (!region_idx.has_value() && BTB_PARTIAL_TAG_RESOLUTION) {
      opt_entry = ::L2_BTB.at(this).check_hit({ip, branch_target, type, std::tuple<uint16_t, uint16_t, uint64_t>{0, 0, 0}}, true);
  ...
  if (branch_target != 0) {
    ...
    if (branch_type != BRANCH_RETURN && opt_entry.has_value() && opt_entry.value().get_prediction() != branch_target) {
      invalidated_entry = ::L2_BTB.at(this).invalidate(opt_entry.value());
    ...
    fill_entry.precise_branch_type = branch_type;
    replaced_entry = ::L2_BTB.at(this).fill(
        fill_entry,
...
void O3_CPU::btb_invalidate_entry(uint64_t ip)
{
  ...
  if (btb_invalidate_entry_on_alias)
    ::L2_BTB.at(this).invalidate(btb_entry.value());
