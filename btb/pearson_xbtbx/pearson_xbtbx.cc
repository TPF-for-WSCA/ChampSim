...
// Simple Pearson hashing implementation producing up to 64 bits by
// concatenating 8-bit Pearson outputs computed with different salts.
static const uint8_t _pearson_table[256] = {
    ...
    154,  35,  86,  171, 105, 34,  38,  200, 147, 58,  77,  118, 126, 255, 216, 83,
    44,   200, 240, 169, 145, 36,  66,  139, 213, 146, 63,  157, 228, 247, 152, 2,
    14,   19,  215, 68,  219, 91,  176, 11,  13,  214, 140, 75,  78,  232, 121, 37,
    64,   70,  227, 180, 231, 92,  11,  242, 31,  223, 216, 104, 71,  54,  41,  170,
    250,  133, 210, 24,  183, 97,  56,  138, 143, 199, 226, 12,  106, 21,  119, 148,
    152,  61,  236, 28,  144, 10,  183, 55,  232, 109, 101, 240, 44,  131, 148, 177,
    96,   188, 52,  27,  84,  126, 3,   34,  43,  117, 158, 47,  222, 134, 25,  46,
    247,  91,  64,  39,  139, 216, 67,  229, 15,  231, 20,  189, 121, 179, 73,  213
};

static uint64_t pearson_hash(uint64_t value, uint8_t out_bits)
{
  if (out_bits == 0)
    return 0;
  uint8_t bytes_needed = (out_bits + 7) / 8;
  uint64_t result = 0;
  for (uint8_t i = 0; i < bytes_needed; ++i) {
    // Pearson hash: initialize with salt i and run over the 8 bytes of value
    uint8_t h = _pearson_table[(uint8_t)i];
    for (int b = 0; b < 8; ++b) {
      uint8_t vb = (value >> (b * 8)) & 0xFF;
      h = _pearson_table[h ^ vb];
    }
    result |= (uint64_t)h << (i * 8);
  }
  if (out_bits < 64) {
    result &= ((1ULL << out_bits) - 1);
  }
  return result;
}
...
auto get_region(uint64_t ip)
{
  ...
  ip = pearson_hash(ip, _BTB_TAG_REGIONS);
  ip = ip & _REGION_MASK;
  return ip;
}
...
struct BTBEntry {
  ...
  auto tag() const
  {
    ...
    tag = pearson_hash(tag, _FULL_TAG_MASK);
    tag &= _FULL_TAG_MASK;
    if (ip && _BTB_TAG_REGIONS && utilise_regions(target_size)) {
      ...
      if (_PERFECT_MAPPING)
        ...
        tag |= (std::get<2>(region_idx_tag) << _BTB_TAG_SIZE); // Precise pointers
    }
    return tag;
  }
...
struct region_btb_entry_t {
  ...
  auto index() const
  {
    ...
    uint64_t region = get_region(ip_tag);
    return region & (_BTB_TAG_REGION_SETS - 1);
  }
  auto tag() const
  {
...
void O3_CPU::update_btb(uint64_t ip, uint64_t branch_target, uint8_t taken, uint8_t branch_type)
{
  ...
  if (branch_target != 0) {
    ...
    if (::BTB.at(this).find_useless(fill_entry)) {
      ...
      if (replaced_entry.has_value() && replaced_entry.value().replacement_protected)
        sim_stats.btb_unforced_useful_evictions--;
    }
