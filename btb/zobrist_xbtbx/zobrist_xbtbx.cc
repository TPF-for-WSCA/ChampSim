...
namespace
{
// We generate a deterministic table of 64 64-bit random values via splitmix64 and XOR the entries for each set bit in the input. The result is then clipped to out_bits
...
static uint64_t zobrist_hash(uint64_t value, uint8_t out_bits)
{
  if (out_bits == 0)
    return 0;
  ...
  if (!initialized) {
    uint64_t seed = 0xfeedbeef12345678ULL; // deterministic seed
...
struct BTBEntry {
  ...
  auto tag() const
  {
    ...
    tag = zobrist_hash(tag, _FULL_TAG_MASK);
    tag &= _FULL_TAG_MASK;
