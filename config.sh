...
)  # doing this early because LLC dimensions depend on it
...
for i in range(len(config_file["ooo_cpu"])):
    config_file["ooo_cpu"][i] = merge_dicts(
        default_core,
        {"branch_predictor": config_file["branch_predictor"]}
        if "branch_predictor" in config_file
        else {},
        config_file["ooo_cpu"][i],
    )
    config_file["ooo_cpu"][i] = merge_dicts(
        default_core,
        {"btb": config_file["btb"]} if "btb" in config_file else {},
        config_file["ooo_cpu"][i],
    )
    config_file["ooo_cpu"][i]["DIB"] = merge_dicts(
        default_dib,
        config_file.get("DIB", {}),
        config_file["ooo_cpu"][i].get("DIB", {}),
    )
    config_file["ooo_cpu"][i]["L1I"] = merge_dicts(
        default_l1i,
        config_file.get("L1I", {}),
        config_file["ooo_cpu"][i].get("L1I", {}),
    )
    config_file["ooo_cpu"][i]["L1D"] = merge_dicts(
        default_l1d,
        config_file.get("L1D", {}),
        config_file["ooo_cpu"][i].get("L1D", {}),
    )
    config_file["ooo_cpu"][i]["L2C"] = merge_dicts(
        default_l2c,
        config_file.get("L2C", {}),
        config_file["ooo_cpu"][i].get("L2C", {}),
    )
    config_file["ooo_cpu"][i]["ITLB"] = merge_dicts(
        default_itlb,
        config_file.get("ITLB", {}),
        config_file["ooo_cpu"][i].get("ITLB", {}),
    )
    config_file["ooo_cpu"][i]["DTLB"] = merge_dicts(
        default_dtlb,
        config_file.get("DTLB", {}),
        config_file["ooo_cpu"][i].get("DTLB", {}),
    )
    config_file["ooo_cpu"][i]["STLB"] = merge_dicts(
        default_stlb,
        config_file.get("STLB", {}),
        config_file["ooo_cpu"][i].get("STLB", {}),
    )

config_file["LLC"] = merge_dicts(default_llc, config_file.get("LLC", {}))
config_file["physical_memory"] = merge_dicts(
    default_pmem, config_file.get("physical_memory", {})
)
config_file["virtual_memory"] = merge_dicts(
    default_vmem, config_file.get("virtual_memory", {})
)
...
for cpu in config_file["ooo_cpu"]:
    cpu["L1I"]["hit_latency"] = cpu["L1I"].get(
        "hit_latency", cpu["L1I"]["latency"] - cpu["L1I"]["fill_latency"]
    )
    cpu["L1D"]["hit_latency"] = cpu["L1D"].get(
        "hit_latency", cpu["L1D"]["latency"] - cpu["L1D"]["fill_latency"]
    )
    cpu["L2C"]["hit_latency"] = cpu["L2C"].get(
        "hit_latency", cpu["L2C"]["latency"] - cpu["L2C"]["fill_latency"]
    )
    cpu["ITLB"]["hit_latency"] = cpu["ITLB"].get(
        "hit_latency", cpu["ITLB"]["latency"] - cpu["ITLB"]["fill_latency"]
    )
    cpu["DTLB"]["hit_latency"] = cpu["DTLB"].get(
        "hit_latency", cpu["DTLB"]["latency"] - cpu["DTLB"]["fill_latency"]
    )
    cpu["STLB"]["hit_latency"] = cpu["STLB"].get(
        "hit_latency", cpu["STLB"]["latency"] - cpu["STLB"]["fill_latency"]
    )

config_file["LLC"]["hit_latency"] = config_file["LLC"].get(
    "hit_latency",
    config_file["LLC"]["latency"] - config_file["LLC"]["fill_latency"],
)
...
config_file["ooo_cpu"] = list(
    itertools.islice(
        itertools.repeat(*config_file["ooo_cpu"]), config_file["num_cores"]
    )
)
...
for i, cpu in enumerate(config_file["ooo_cpu"][:1]):
    if cpu["L1I"]["prefetcher"] is not None:
        libfilenames["cpu" + str(i) + "l1iprefetcher.a"] = (
            "prefetcher/" + cpu["L1I"]["prefetcher"]
        )
    if cpu["L1D"]["prefetcher"] is not None:
        libfilenames["cpu" + str(i) + "l1dprefetcher.a"] = (
            "prefetcher/" + cpu["L1D"]["prefetcher"]
        )
    if cpu["L2C"]["prefetcher"] is not None:
        libfilenames["cpu" + str(i) + "l2cprefetcher.a"] = (
            "prefetcher/" + cpu["L2C"]["prefetcher"]
        )
    if cpu["branch_predictor"] is not None:
        libfilenames["cpu" + str(i) + "branch_predictor.a"] = (
            "branch/" + cpu["branch_predictor"]
        )
    if cpu["btb"] is not None:
        libfilenames["cpu" + str(i) + "btb.a"] = "btb/" + cpu["btb"]
if config_file["LLC"]["prefetcher"] is not None:
    libfilenames["llprefetcher.a"] = (
        "prefetcher/" + config_file["LLC"]["prefetcher"]
    )
if config_file["LLC"]["replacement"] is not None:
    libfilenames["llreplacement.a"] = (
        "replacement/" + config_file["LLC"]["replacement"]
    )
...
with open(constants_header_name, "wt") as wfp:
    ...
    wfp.write(
        define_fmtstr.format(name="cpu_clock_freq").format(
            names=const_names, config=config_file
        )
    )
    ...
    for k, v in config_file["ooo_cpu"][0].items():
        if isinstance(v, dict):
            if k == "DIB":
                wfp.write(
                    define_fmtstr.format(name="window_size").format(
                        names=const_names["core"]["DIB"],
                        config=config_file["ooo_cpu"][0]["DIB"],
                    )
                )
                wfp.write(
                    define_log_fmtstr.format(name="window_size").format(
                        names=const_names["core"]["DIB"],
                        config=config_file["ooo_cpu"][0]["DIB"],
                    )
                )
                wfp.write(
                    define_fmtstr.format(name="sets").format(
                        names=const_names["core"]["DIB"],
                        config=config_file["ooo_cpu"][0]["DIB"],
                    )
                )
                wfp.write(
                    define_fmtstr.format(name="ways").format(
                        names=const_names["core"]["DIB"],
                        config=config_file["ooo_cpu"][0]["DIB"],
                    )
                )
            ...
            if k != "branch_predictor" and k != "btb":
                wfp.write(
                    define_fmtstr.format(name=k).format(
                        names=const_names["core"],
                        config=config_file["ooo_cpu"][0],
                    )
                )

    wfp.write(
        cache_define_fmtstr.format(name="LLC", attrs=config_file["LLC"]) + "\n"
    )
    ...
    for k in const_names["physical_memory"]:
        ...
        if k in ["channels", "ranks", "banks", "rows", "columns"]:
            wfp.write(
                define_log_fmtstr.format(name=k).format(
                    names=const_names["physical_memory"],
                    config=config_file["physical_memory"],
                )
            )
