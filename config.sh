...
cache_fmtstr = 'CACHE {name}("{name}", {frequency}, {fill_level}, {sets}, {ways}, {perfect}, {wq_size}, {rq_size}, {pq_size}, {mshr_size}, {hit_latency}, {fill_latency}, {max_read}, {max_write}, {offset_bits}, {prefetch_as_load:b}, {wq_check_full_addr:b}, {virtual_prefetch:b}, {prefetch_activate_mask}, {lower_level}, CACHE::pref_t::{prefetcher_name}, CACHE::repl_t::{replacement_name}, CountBlockMethod::{counting_block_method}, LruModifier::{lru_modifier}, {filter_inserts:b}, {filter_prefetches:b},  {filter_buffer_size}, {prefetch_buffer_size});\n'
vlc_cache_fmtstr = 'VCL_CACHE {name}("{name}", {frequency}, {fill_level}, {sets}, {ways}, way_sizes, {buffer:b}, {buffer_size}, {buffer_fifo:b}, {aligned:b}, {wq_size}, {rq_size}, {pq_size}, {mshr_size}, {hit_latency}, {fill_latency}, {max_read}, {max_write}, {offset_bits}, {prefetch_as_load:b}, {wq_check_full_addr:b}, {virtual_prefetch:b}, {prefetch_activate_mask}, {lower_level}, CACHE::pref_t::{prefetcher_name}, CACHE::repl_t::{replacement_name}, BufferOrganisation::{buffer_organisation}, LruModifier::{lru_modifier}, CountBlockMethod::{counting_block_method}, BufferHistory::{buffer_history}, {filter_inserts:b}, {filter_prefetches:b}, {filter_buffer_size}, {prefetch_buffer_size}, {extend_blocks_to_branch:b}, CacheType::{insertion_method}, ReplacementStrategy::{replacement_strategy}, {word_size});\n'
...
default_l1i = {
    ...
    "prefetcher": "no_instr",
    "replacement": "lru",
    "lru_modifier": "DEFAULT",
    "counting_block_method": "EVICTION",
    "buffer_history": "FULL",
    "filter_inserts": False,
    "filter_prefetches": False,
    "filter_buffer_size": 64,
...
default_llc = {
    "sets": 2048 * config_file["num_cores"],
    "ways": 16,
    "rq_size": 32 * config_file["num_cores"],
    "wq_size": 32 * config_file["num_cores"],
    "pq_size": 32 * config_file["num_cores"],
    "mshr_size": 64 * config_file["num_cores"],
    "latency": 20,
    "fill_latency": 1,
    "max_read": config_file["num_cores"],
    "max_write": config_file["num_cores"],
    "prefetch_as_load": False,
    "virtual_prefetch": False,
    "wq_check_full_addr": False,
    "prefetch_activate": "LOAD,PREFETCH",
...
config_file["physical_memory"] = ChainMap(
    config_file["physical_memory"], default_pmem.copy()
)
config_file["virtual_memory"] = ChainMap(
    config_file["virtual_memory"], default_vmem.copy()
)
...
for i in range(len(cores)):
    cores[i] = ChainMap(
        cores[i],
        {"name": "cpu" + str(i), "index": i},
        copy.deepcopy(
            dict(
                (k, v) for k, v in config_file.items() if k not in ("ooo_cpu", "cache")
            )
        ),
        default_core.copy(),
    )
    cores[i]["DIB"] = ChainMap(
        cores[i]["DIB"], config_file["DIB"].copy(), default_dib.copy()
    )
...
caches["LLC"] = ChainMap(
    caches.get("LLC", {}),
    config_file["LLC"].copy(),
    {"frequency": max(cpu["frequency"] for cpu in cores)},
    default_llc.copy(),
)
...
for cpu in cores:
    ...
    for cache_name in ("L1I", "L1D", "L2C", "ITLB", "DTLB", "STLB"):
        if isinstance(cpu[cache_name], dict):
            cpu[cache_name] = ChainMap(
                cpu[cache_name],
                {"name": cpu["name"] + "_" + cache_name},
                config_file[cache_name].copy(),
            )
...
for cpu in cores:
    ...
    caches[cpu["L1I"]] = ChainMap(
        caches[cpu["L1I"]],
        {"frequency": cpu["frequency"], "lower_level": cpu["L2C"]},
        default_l1i.copy(),
    )
    caches[cpu["L1D"]] = ChainMap(
        caches[cpu["L1D"]],
        {"frequency": cpu["frequency"], "lower_level": cpu["L2C"]},
        default_l1d.copy(),
    )
    caches[cpu["ITLB"]] = ChainMap(
        caches[cpu["ITLB"]],
        {"frequency": cpu["frequency"], "lower_level": cpu["STLB"]},
        default_itlb.copy(),
    )
    caches[cpu["DTLB"]] = ChainMap(
        caches[cpu["DTLB"]],
        {"frequency": cpu["frequency"], "lower_level": cpu["STLB"]},
        default_dtlb.copy(),
    )
    ...
    if cache_name != "DRAM":
        caches[cache_name] = ChainMap(
            caches[cache_name],
            {"frequency": cpu["frequency"], "lower_level": "LLC"},
            default_l2c.copy(),
        )
    ...
    if cache_name != "DRAM":
        caches[cache_name] = ChainMap(
            caches[cache_name],
            {"frequency": cpu["frequency"], "lower_level": cpu["PTW"]["name"]},
            default_stlb.copy(),
        )
...
for i, ll in enumerate(caches.values()):
    accessible[i] |= any(
        ul["lower_level"] == ll["name"] for ul in caches.values()
    )  # The cache is accessible from another cache
    accessible[i] |= any(
        ll["name"] in [cpu["L1I"], cpu["L1D"], cpu["ITLB"], cpu["DTLB"]]
        for cpu in cores
    )  # The cache is accessible from a core
caches = dict(itertools.compress(caches.items(), accessible))

# Establish latencies in caches
for cache in caches.values():
    cache["hit_latency"] = cache.get("hit_latency") or (
        cache["latency"] - cache["fill_latency"]
    )

# Create prefetch activation masks
type_list = ("LOAD", "RFO", "PREFETCH", "WRITEBACK", "TRANSLATION")
for cache in caches.values():
    cache["prefetch_activate_mask"] = functools.reduce(
        operator.or_,
        (
            1 << i
...
# Scale frequencies
config_file["physical_memory"]["io_freq"] = config_file["physical_memory"][
    "frequency"
]  # Save value
freqs = list(
    itertools.chain(
...
for freq, src in zip(
    freqs, itertools.chain(cores, caches.values(), (config_file["physical_memory"],))
):
    src["frequency"] = freq
...
for cache in caches.values():
    ...
    if cache["replacement"] is not None:
        ...
        cache["replacement_initialize"] = (
            "repl_" + cache["replacement_name"] + "_initialize"
        )
        cache["replacement_find_victim"] = (
            "repl_" + cache["replacement_name"] + "_victim"
        )
        cache["replacement_update_replacement_state"] = (
            "repl_" + cache["replacement_name"] + "_update"
        )
        cache["replacement_replacement_final_stats"] = (
            "repl_" + cache["replacement_name"] + "_final_stats"
        )
        ...
        opts += (
            " -Dupdate_replacement_state="
            + cache["replacement_update_replacement_state"]
        )
        opts += (
            " -Dreplacement_final_stats=" + cache["replacement_replacement_final_stats"]
        )
    ...
    if cache["prefetcher"] is not None:
        ...
        cache["prefetcher_initialize"] = (
            "pref_" + cache["prefetcher_name"] + "_initialize"
        )
        cache["prefetcher_cache_operate"] = (
            "pref_" + cache["prefetcher_name"] + "_cache_operate"
        )
        cache["prefetcher_cache_fill"] = (
            "pref_" + cache["prefetcher_name"] + "_cache_fill"
        )
        cache["prefetcher_cycle_operate"] = (
            "pref_" + cache["prefetcher_name"] + "_cycle_operate"
        )
        cache["prefetcher_final_stats"] = (
            "pref_" + cache["prefetcher_name"] + "_final_stats"
        )
...
<<<<<<< HEAD
    ...
    cpu["iprefetcher_branch_operate"] = (
        "pref_" + cpu["iprefetcher_name"] + "_branch_operate"
    )
    cpu["iprefetcher_cache_operate"] = (
        "pref_" + cpu["iprefetcher_name"] + "_cache_operate"
    )
    cpu["iprefetcher_cycle_operate"] = (
        "pref_" + cpu["iprefetcher_name"] + "_cycle_operate"
    )
    ...
    caches[cpu["L1I"]]["prefetcher_name"] = (
        "CPU_REDIRECT_" + cpu["iprefetcher_name"] + "_"
    )
...
active_keys = list(
    itertools.chain.from_iterable(
        (cpu["ITLB"], cpu["DTLB"], cpu["L1I"], cpu["L1D"]) for cpu in cores
    )
)
...
for fill_level in range(1, len(memory_system) + 1):
    for k in active_keys:
        if memory_system[k]["lower_level"] != "DRAM":
            memory_system[memory_system[k]["lower_level"]]["fill_level"] = max(
                memory_system[memory_system[k]["lower_level"]].get("fill_level", 0),
                fill_level + 1,
            )
    active_keys = [
        memory_system[k]["lower_level"]
        for k in active_keys
        if memory_system[k]["lower_level"] != "DRAM"
    ]
...
with open(instantiation_file_name, "wt") as wfp:
    ...
    for elem in memory_system:
        ...
        elif "way_sizes" in elem:
            wfp.write(
                f'uint8_t way_sizes[{len(elem["way_sizes"])}] = {{{", ".join("{}".format(val) for val in elem["way_sizes"])}}};\n'
            )
            if "aligned" not in elem:
                elem["aligned"] = False
            if "buffer" not in elem:
                elem["buffer"] = False
            if "buffer_fifo" not in elem:
                elem["buffer_fifo"] = False
            wfp.write(vlc_cache_fmtstr.format(**elem))
        else:
            if "perfect" in elem and elem["perfect"]:
                elem["perfect"] = 1
            else:
                elem["perfect"] = 0
    ...
    wfp.write(
        ", ".join(
            ...
            for elem in itertools.chain(
                cores, memory_system, (config_file["physical_memory"],)
            )
        )
    )
    wfp.write("\n};\n")
...
ipref_inits = {(c["iprefetcher_name"], c["iprefetcher_initialize"]) for c in cores}
ipref_branch_ops = {
    (c["iprefetcher_name"], c["iprefetcher_branch_operate"]) for c in cores
}
ipref_cache_ops = {
    (c["iprefetcher_name"], c["iprefetcher_cache_operate"]) for c in cores
}
ipref_cycle_ops = {
    (c["iprefetcher_name"], c["iprefetcher_cycle_operate"]) for c in cores
}
ipref_fill = {(c["iprefetcher_name"], c["iprefetcher_cache_fill"]) for c in cores}
ipref_finals = {(c["iprefetcher_name"], c["iprefetcher_final_stats"]) for c in cores}
with open("inc/ooo_cpu_modules.inc", "wt") as wfp:
...
>>>>>>> micro-rebuttal
    ...
    wfp.write(
        "\n    ".join(
            "if (bpred_type == bpred_t::{}) return {}();".format(*b)
            for b in bpred_inits
        )
    )
    ...
    wfp.write(
        "\n".join(
            "void {1}(uint64_t, uint64_t, uint8_t, uint8_t);".format(*b)
            for b in bpred_last_results
        )
    )
    wfp.write(
    ...
    wfp.write(
        "\n    ".join(
            "if (bpred_type == bpred_t::{}) return {}(ip, target, taken, branch_type);".format(
                *b
            )
            for b in bpred_last_results
        )
    ...
    wfp.write(
        "\n".join(
            "uint8_t {1}(uint64_t, uint64_t, uint8_t, uint8_t);".format(*b)
            for b in bpred_predicts
        )
    )
    wfp.write(
    ...
    wfp.write(
        "\n    ".join(
            "if (bpred_type == bpred_t::{}) return {}(ip, predicted_target, always_taken, branch_type);".format(
                *b
            )
            for b in bpred_predicts
        )
    ...
    wfp.write(
        "\n    ".join(
            "if (btb_type == btb_t::{}) return {}();".format(*b) for b in btb_inits
        )
    )
    wfp.write(
    ...
    wfp.write(
        "\n    ".join(
            "if (btb_type == btb_t::{}) return {}(ip, branch_target, taken, branch_type);".format(
                *b
            )
            for b in btb_updates
        )
    ...
    wfp.write(
        "\n".join(
            "BTB_outcome {1}(uint64_t, uint8_t);".format(*b) for b in btb_predicts
        )
    )
    wfp.write(
    ...
    wfp.write(
        "\n    ".join(
            "if (btb_type == btb_t::{}) return {}(ip, branch_type);".format(*b)
            for b in btb_predicts
        )
    )
    wfp.write(
        '\n    throw std::invalid_argument("Branch target buffer module not found");'
...
>>>>>>> micro-rebuttal
    ...
    wfp.write(
        "\n".join(
            "void {1}(uint64_t, uint8_t, uint64_t, uint8_t);".format(*i)
            for i in ipref_branch_ops
        )
    )
    wfp.write(
    ...
    wfp.write(
        "\n    ".join(
            "if (ipref_type == ipref_t::{}) return {}(ip, branch_type, branch_target, size);".format(
                *i
            )
            for i in ipref_branch_ops
        )
    ...
    wfp.write(
        "\n".join(
            "uint32_t {1}(uint64_t, uint8_t, uint8_t, uint32_t);".format(*i)
            for i in ipref_cache_ops
        )
    )
    ...
    wfp.write(
        "\n    ".join(
            "if (ipref_type == ipref_t::{}) return {}();".format(*i)
            for i in ipref_cycle_ops
        )
    )
    wfp.write(
        '\n    throw std::invalid_argument("Instruction prefetcher module not found");'
    ...
    wfp.write(
        "\n".join(
            "uint32_t {1}(uint64_t, uint32_t, uint32_t, uint8_t, uint64_t, uint32_t);".format(
                *i
            )
            for i in ipref_fill
        )
...
repl_inits = {
    (c["replacement_name"], c["replacement_initialize"]) for c in caches.values()
}
repl_victims = {
    (c["replacement_name"], c["replacement_find_victim"]) for c in caches.values()
}
repl_updates = {
    (c["replacement_name"], c["replacement_update_replacement_state"])
    for c in caches.values()
}
repl_finals = {
    (c["replacement_name"], c["replacement_replacement_final_stats"])
    for c in caches.values()
}
pref_names = {c["prefetcher_name"] for c in caches.values()}
pref_inits = {
    (c["prefetcher_name"], c["prefetcher_initialize"]) for c in caches.values()
}
pref_ops = {
    (c["prefetcher_name"], c["prefetcher_cache_operate"]) for c in caches.values()
}
pref_fill = {
    (c["prefetcher_name"], c["prefetcher_cache_fill"]) for c in caches.values()
}
pref_cycles = {
    (c["prefetcher_name"], c["prefetcher_cycle_operate"]) for c in caches.values()
}
pref_finals = {
    (c["prefetcher_name"], c["prefetcher_final_stats"]) for c in caches.values()
}
with open("inc/cache_modules.inc", "wt") as wfp:
    ...
    wfp.write(
        "\n    ".join(
            "if (repl_type == repl_t::{}) return {}();".format(*r) for r in repl_inits
        )
    )
    wfp.write(
    ...
    wfp.write(
        "\n".join(
            "uint32_t {1}(uint32_t, uint64_t, uint32_t, const BLOCK*, uint64_t, uint64_t, uint32_t);".format(
                *r
            )
            for r in repl_victims
        )
    )
    wfp.write(
    ...
    wfp.write(
        "\n    ".join(
            "if (repl_type == repl_t::{}) return {}(cpu, instr_id, set, current_set, ip, full_addr, type);".format(
                *r
            )
            for r in repl_victims
        )
    ...
    wfp.write(
        "\n".join(
            "void {1}(uint32_t, uint32_t, uint32_t, uint64_t, uint64_t, uint64_t, uint32_t, uint8_t);".format(
                *r
            )
            for r in repl_updates
        )
    )
    wfp.write(
    ...
    wfp.write(
        "\n    ".join(
            "if (repl_type == repl_t::{}) return {}(cpu, set, way, full_addr, ip, victim_addr, type, hit);".format(
                *r
            )
            for r in repl_updates
        )
    ...
    wfp.write(
        "\n    ".join(
            "if (repl_type == repl_t::{}) return {}();".format(*r) for r in repl_finals
        )
    )
    wfp.write(
        '\n    throw std::invalid_argument("Replacement policy module not found");'
    ...
    wfp.write(
        "\n".join(
            "void {1}();".format(*p)
            for p in pref_inits
            if not p[0].startswith("CPU_REDIRECT")
        )
    )
    wfp.write("\nvoid impl_prefetcher_initialize()\n{\n    ")
    pref_inits = {
        (n, ("ooo_cpu[cpu]->" if n.startswith("CPU_REDIRECT") else "") + f)
        for n, f in pref_inits
    }  ## prepend redirect
    wfp.write(
        "\n    ".join(
            "if (pref_type == pref_t::{}) return {}();".format(*p) for p in pref_inits
        )
    )
    ...
    wfp.write(
        "\n".join(
            "uint32_t {1}(uint64_t, uint64_t, uint8_t, uint8_t, uint32_t);".format(*p)
            for p in pref_ops
            if not p[0].startswith("CPU_REDIRECT")
        )
    )
    ...
    pref_ops = {
        (
            n,
            (
                "ooo_cpu[cpu]->{}(addr, cache_hit, (type == PREFETCH), metadata_in)"
                if n.startswith("CPU_REDIRECT")
                else "{}(addr, ip, cache_hit, type, metadata_in)"
            ).format(f),
        )
        for n, f in pref_ops
    }  ## modify signature for redirect
    wfp.write(
        "\n    ".join(
            "if (pref_type == pref_t::{}) return {};".format(*p) for p in pref_ops
        )
    )
    wfp.write('\n    throw std::invalid_argument("Data prefetcher module not found");')
    wfp.write("\n}\n")
    ...
    pref_fill = {
        (n, ("ooo_cpu[cpu]->" if n.startswith("CPU_REDIRECT") else "") + f)
        for n, f in pref_fill
    }  ## prepend redirect
    wfp.write(
        "\n    ".join(
            "if (pref_type == pref_t::{}) return {}(addr, set, way, prefetch, evicted_addr, metadata_in);".format(
                *p
            )
            for p in pref_fill
        )
    ...
    pref_cycles = {
        (n, ("ooo_cpu[cpu]->" if n.startswith("CPU_REDIRECT") else "") + f)
        for n, f in pref_cycles
    }  ## prepend redirect
    wfp.write(
        "\n    ".join(
            "if (pref_type == pref_t::{}) return {}();".format(*p) for p in pref_cycles
        )
    )
    wfp.write('\n    throw std::invalid_argument("Data prefetcher module not found");')
    wfp.write("\n}\n")
    ...
    pref_finals = {
        (n, ("ooo_cpu[cpu]->" if n.startswith("CPU_REDIRECT") else "") + f)
        for n, f in pref_finals
    }  ## prepend redirect
    wfp.write(
        "\n    ".join(
            "if (pref_type == pref_t::{}) return {}();".format(*p) for p in pref_finals
        )
    )
    wfp.write('\n    throw std::invalid_argument("Data prefetcher module not found");')
    wfp.write("\n}\n")
...
with open(constants_header_name, "wt") as wfp:
    ...
    wfp.write(
        define_fmtstr.format(name="block_size").format(
            names=const_names, config=config_file
        )
    )
    wfp.write(
        define_log_fmtstr.format(name="block_size").format(
            names=const_names, config=config_file
        )
    )
    wfp.write(
        define_fmtstr.format(name="bucket_size").format(
            names=const_names, config=config_file
        )
    )
    wfp.write(
        define_fmtstr.format(name="page_size").format(
            names=const_names, config=config_file
        )
    )
    wfp.write(
        define_log_fmtstr.format(name="page_size").format(
            names=const_names, config=config_file
        )
    )
    wfp.write(
        define_fmtstr.format(name="heartbeat_frequency").format(
            names=const_names, config=config_file
        )
    )
    wfp.write(
        define_fmtstr.format(name="num_cores").format(
            names=const_names, config=config_file
        )
    )
    wfp.write("#define NUM_CACHES " + str(len(caches)) + "u\n")
    wfp.write(
        "#define NUM_OPERABLES " + str(len(cores) + len(memory_system) + 1) + "u\n"
    )

    for k in const_names["L1I"]:
        wfp.write(
            define_fmtstr.format(name=k).format(
                names=const_names["L1I"], config=config_file["L1I"]
            )
        )
    ...
    for k in const_names["physical_memory"]:
        if k in ["tRP", "tRCD", "tCAS", "turn_around_time"]:
            wfp.write(
                define_nonint_fmtstr.format(name=k).format(
                    names=const_names["physical_memory"],
                    config=config_file["physical_memory"],
                )
            )
        else:
            wfp.write(
...
with open("Makefile", "wt") as wfp:
    ...
    wfp.write(
        "CXXFLAGS := " + config_file.get("CXXFLAGS", "-Wall -O3") + " -std=c++17\n"
    )
    ...
    for v in libfilenames.values():
        wfp.write(
            "\t find {0} -name \*.o -delete\n\t find {0} -name \*.d -delete\n".format(
                *v
            )
        )
    wfp.write("\n")
    wfp.write(
        config_file["executable_name"]
        + ": $(patsubst %.cc,%.o,$(wildcard src/*.cc)) "
        + " ".join("obj/" + k for k in libfilenames)
        + "\n"
    )
