...
for freq, src in zip(
    freqs,
    itertools.chain(cores, caches.values(), (config_file["physical_memory"],)),
):
    src["frequency"] = freq
...
for cache in caches.values():
    ...
    if cache["replacement"] is not None:
        ...
        cache["replacement_name"] = "r" + fname.translate(
            fname_translation_table
        )
        ...
        opts += (
            " -Dreplacement_final_stats="
            + cache["replacement_replacement_final_stats"]
        )
        libfilenames["repl_" + cache["replacement_name"] + ".a"] = (
            fname,
            opts,
        )
    ...
    if cache["prefetcher"] is not None:
        ...
        cache["prefetcher_name"] = "p" + fname.translate(
            fname_translation_table
        )
        ...
        opts += (
            " -Dprefetcher_cache_operate=" + cache["prefetcher_cache_operate"]
        )
        opts += " -Dprefetcher_cache_fill=" + cache["prefetcher_cache_fill"]
        opts += (
            " -Dprefetcher_cycle_operate=" + cache["prefetcher_cycle_operate"]
        )
        opts += " -Dprefetcher_final_stats=" + cache["prefetcher_final_stats"]
        # These function names are deprecated, but we still permit them
        opts += (
            " -Dl1d_prefetcher_initialize=" + cache["prefetcher_initialize"]
        )
        opts += (
            " -Dl2c_prefetcher_initialize=" + cache["prefetcher_initialize"]
        )
        opts += (
            " -Dllc_prefetcher_initialize=" + cache["prefetcher_initialize"]
        )
        opts += (
            " -Dl1d_prefetcher_operate=" + cache["prefetcher_cache_operate"]
        )
        opts += (
            " -Dl2c_prefetcher_operate=" + cache["prefetcher_cache_operate"]
        )
        opts += (
            " -Dllc_prefetcher_operate=" + cache["prefetcher_cache_operate"]
        )
        opts += (
            " -Dl1d_prefetcher_cache_fill=" + cache["prefetcher_cache_fill"]
        )
        opts += (
            " -Dl2c_prefetcher_cache_fill=" + cache["prefetcher_cache_fill"]
        )
        opts += (
            " -Dllc_prefetcher_cache_fill=" + cache["prefetcher_cache_fill"]
        )
        opts += (
            " -Dl1d_prefetcher_final_stats=" + cache["prefetcher_final_stats"]
        )
        opts += (
            " -Dl2c_prefetcher_final_stats=" + cache["prefetcher_final_stats"]
        )
        opts += (
            " -Dllc_prefetcher_final_stats=" + cache["prefetcher_final_stats"]
        )
...
for cpu in cores:
    ...
    if cpu["branch_predictor"] is not None:
        ...
        cpu["bpred_last_result"] = (
            "bpred_" + cpu["bpred_name"] + "_last_result"
        )
    ...
    cpu["iprefetcher_initialize"] = (
        "pref_" + cpu["iprefetcher_name"] + "_initialize"
    )
    ...
    cpu["iprefetcher_cache_fill"] = (
        "pref_" + cpu["iprefetcher_name"] + "_cache_fill"
    )
    cpu["iprefetcher_final_stats"] = (
        "pref_" + cpu["iprefetcher_name"] + "_final_stats"
    )
    ...
    opts += (
        " -Dl1i_prefetcher_branch_operate=" + cpu["iprefetcher_branch_operate"]
    )
    opts += (
        " -Dl1i_prefetcher_cache_operate=" + cpu["iprefetcher_cache_operate"]
    )
    opts += (
        " -Dl1i_prefetcher_cycle_operate=" + cpu["iprefetcher_cycle_operate"]
    )
    ...
    caches[cpu["L1I"]]["prefetcher_cache_operate"] = cpu[
        "iprefetcher_cache_operate"
    ]
    caches[cpu["L1I"]]["prefetcher_cache_fill"] = cpu["iprefetcher_cache_fill"]
    caches[cpu["L1I"]]["prefetcher_cycle_operate"] = cpu[
        "iprefetcher_cycle_operate"
    ]
    caches[cpu["L1I"]]["prefetcher_final_stats"] = cpu[
        "iprefetcher_final_stats"
    ]
...
for f in os.listdir("obj"):
    if (
        f in libfilenames
        and f in config_cache
        and config_cache[f] != libfilenames[f]
    ):
        os.remove("obj/" + f)
    ...
    for k in active_keys:
        if memory_system[k]["lower_level"] != "DRAM":
            memory_system[memory_system[k]["lower_level"]]["fill_level"] = max(
                memory_system[memory_system[k]["lower_level"]].get(
                    "fill_level", 0
                ),
                fill_level + 1,
            )
...
for i in reversed(range(len(memory_system))):
    ...
    if ul["lower_level"] != "DRAM":
        if not any(
            (ul["lower_level"] == ll["name"]) for ll in memory_system[:i]
        ):
            print(
...
ipref_inits = {
    (c["iprefetcher_name"], c["iprefetcher_initialize"]) for c in cores
}
ipref_branch_ops = {
...
ipref_fill = {
    (c["iprefetcher_name"], c["iprefetcher_cache_fill"]) for c in cores
}
ipref_finals = {
    (c["iprefetcher_name"], c["iprefetcher_final_stats"]) for c in cores
}
with open("inc/ooo_cpu_modules.inc", "wt") as wfp:
    ...
    wfp.write(
        "\n".join(
            "std::pair<uint64_t, uint8_t> {1}(uint64_t, uint8_t);".format(*b)
    ...
    wfp.write(
        "\n".join(
            "void {1}(uint64_t, uint8_t, uint64_t);".format(*i)
    ...
    wfp.write(
        "\n    ".join(
            "if (ipref_type == ipref_t::{}) return {}(ip, branch_type, branch_target);".format(
                *i
            )
            for i in ipref_branch_ops
        )
...
repl_inits = {
    (c["replacement_name"], c["replacement_initialize"])
    for c in caches.values()
}
repl_victims = {
    (c["replacement_name"], c["replacement_find_victim"])
    for c in caches.values()
}
repl_updates = {
...
pref_ops = {
    (c["prefetcher_name"], c["prefetcher_cache_operate"])
    for c in caches.values()
}
pref_fill = {
...
pref_cycles = {
    (c["prefetcher_name"], c["prefetcher_cycle_operate"])
    for c in caches.values()
}
pref_finals = {
    (c["prefetcher_name"], c["prefetcher_final_stats"])
    for c in caches.values()
}
with open("inc/cache_modules.inc", "wt") as wfp:
    ...
    wfp.write(
        "\n    ".join(
            "if (repl_type == repl_t::{}) return {}();".format(*r)
            for r in repl_inits
        )
    )
    wfp.write(
    ...
    wfp.write(
        "\n    ".join(
            "if (pref_type == pref_t::{}) return {}();".format(*p)
            for p in pref_inits
        )
    )
    wfp.write(
    ...
    wfp.write(
        "\n".join(
            "uint32_t {1}(uint64_t, uint64_t, uint8_t, uint8_t, uint32_t);".format(
                *p
            )
            for p in pref_ops
            if not p[0].startswith("CPU_REDIRECT")
...
with open(constants_header_name, "wt") as wfp:
    ...
    wfp.write(
        "#define NUM_OPERABLES "
        + str(len(cores) + len(memory_system) + 1)
        + "u\n"
    )
...
with open("Makefile", "wt") as wfp:
    ...
    wfp.write(
        "CFLAGS := " + config_file.get("CFLAGS", "-Wall -O3") + " -std=gnu99\n"
    )
    wfp.write(
        "CXXFLAGS := "
        + config_file.get("CXXFLAGS", "-Wall -O3")
        + " -std=c++17\n"
    )
    wfp.write(
        "CPPFLAGS := " + config_file.get("CPPFLAGS", "") + " -Iinc -MMD -MP\n"
    )
