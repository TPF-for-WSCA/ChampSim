...
vlc_cache_fmtstr = 'VCL_CACHE {name}("{name}", {frequency}, {fill_level}, {sets}, {ways}, way_sizes, {buffer:b}, {buffer_size}, {buffer_fifo:b}, {aligned:b}, {wq_size}, {rq_size}, {pq_size}, {mshr_size}, {hit_latency}, {fill_latency}, {max_read}, {max_write}, {offset_bits}, {prefetch_as_load:b}, {wq_check_full_addr:b}, {virtual_prefetch:b}, {prefetch_activate_mask}, {lower_level}, CACHE::pref_t::{prefetcher_name}, CACHE::repl_t::{replacement_name}, BufferOrganisation::{buffer_organisation}, LruModifier::{lru_modifier}, {perfect_predictor:b});\n'
    ...
    for elem in memory_system:
        if 'pscl5_set' in elem:
            ...
            if 'buffer' not in elem:
                elem["buffer"] = False
            if 'perfect_predictor' not in elem:
                elem["perfect_predictor"] = False
