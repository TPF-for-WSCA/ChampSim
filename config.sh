...
with open(instantiation_file_name, "wt") as wfp:
    ...
    offset_sets_set = False
    for cpu in cores:
        for elem in cpu:
            if "btb_offset_sizes" in elem and not offset_sizes_set:
                wfp.write(
                    f'uint8_t btb_offset_sizes[{len(cpu["btb_offset_sizes"])}] = {{{", ".join("{}".format(val) for val in cpu["btb_offset_sizes"])}}};\n'
                )
                offset_sizes_set = True
            if "offset_btb_sets" in elem and not offset_sets_set:
                wfp.write(
                    f'uint32_t offset_btb_sets[{len(cpu["offset_btb_sets"])}] = {{{", ".join("{}".format(val) for val in cpu["offset_btb_sets"])}}};\n'
                )
                offset_sets_set = True
