...
build_configs=("saga_ubs_x86.json")
cd /cluster/projects/nn4650k/workspace/ChampSim/
old_dir=$(pwd)
for build_script in ${build_configs[@]}
do
    echo "Building ${build_script}"
    /cluster/projects/nn4650k/workspace/ChampSim/config.sh /cluster/projects/nn4650k/workspace/ChampSim/SAGA_CONFIGS/rebuttal-configs/${build_script}
    make -j
done
cd $old_dir
