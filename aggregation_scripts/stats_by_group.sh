...
do
    ...
    ${pg_dir}plotgen --debug -i ./${b}/aliasing.tsv --drop-nan --palette bright --apply-function cset = nan 1 --apply-icolumns : --x-type category --y-tick-format ',.2%' --plot bar --transpose --sort-function name --sort-columns --row-names --renameregex '(.*)\..*trace' --add-function mean --add-row AVG --file ./raw_data/ipc_relative_${b}.tsv  --width 1350 --height 300 -o ./graphs/alias_relative_${b}.html ./graphs/alias_relative_${b}.pdf  &
