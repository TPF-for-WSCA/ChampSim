...
do
    ...
    ~/plotgen/plotgen --debug -i ./${b}/sizes_champsim_data_32k/**/cpu0_L1I_num_cl_with_block_size.tsv --column-names --filename --apply-func cumsum --apply-columns : --normalise-function max --normalise-icolumns : --column-names --renameregex '\./.*/.*/([a-zA-Z\-_0-9\.]+)\.champsimtrace/\.*' --join index --add-function mean --add-column AVG --print --y-tick-format ',.2%' --palette bright --file ./raw_data/accumulated_all_applications_${b}.tsv -o ./graphs/accumulated_all_applications_${b}.html --width 900 --height 200 -o ./graphs/accumulated_all_applications_${b}.pdf 
done
