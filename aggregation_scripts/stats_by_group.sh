...
do
    ...
    ${pg_dir}plotgen --debug -i ./${b}/partial_misses.tsv --drop-nan --group-by-irows 0 --palette bright --x-type category --plot bar --file ./raw_data/partial_detail_${b}.tsv -o ./graphs/partial_detail_${b}.html --width 1350 --height 300 -o ./graphs/partial_detail_${b}.pdf 
    ...
    ${pg_dir}plotgen --debug -i ./${b}/sizes_champsim_data_32k/**/cpu0_L1I_num_cl_with_block_size_accesses_scaled.tsv --column-names --filename --apply-func cumsum --apply-columns : --normalise-function max --normalise-icolumns : --column-names --renameregex '\./.*/.*/([a-zA-Z\-_0-9\.]+)\.champsimtrace/\.*' --join index --add-function mean --add-column AVG --print --x-master-title "Useful Bytes in Cacheline" --y-master-title "% of Cachelines" --y-tick-format ',.2%' --legend-hide --palette bright --y-title-standoff 135 --file ./raw_data/accumulated_all_applications_${b}.tsv -o ./graphs/accumulated_all_applications_${b}.html --width 1350 --height 300 -o ./graphs/accumulated_all_applications_${b}.pdf 
done
