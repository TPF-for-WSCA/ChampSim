...
for ((i=1;i<=64;i++)); do
    ${pg_dir}plotgen -i ./**/**/**/cpu0_size${i}_offset_count.tsv --no-columns --column-names --filename --column-names --renameregex '\./(.*)/.*/([a-zA-Z\-_0-9\.]+)/\.*' --normalise-function sum --normalise-columns : --join index --apply-function cset = nan 0 --apply-icolumns : --sort-rows --plot line --file ./raw_data/ordered_offset_${i}.tsv  --palette bright -o ./graphs/ordered_offset_${i}.html &
done
