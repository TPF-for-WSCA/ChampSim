...
BRANCH_PROGRESS_PAPER_SMALL_FIGSIZE = (BRANCH_PROGRESS_IEEE_TEXT_WIDTH_IN / 2, 2.1)
...
def plot_branch_progress_small_paper_metric(
    ...
    ax.set_xlabel("Sample after Context Switch", fontsize=BRANCH_PROGRESS_PAPER_FONT_SIZE)
...
def plot_branch_progress_paper_metric_svg(rows, metric, ylabel, output_path):
  ...
  {"".join(lines)}
  <text x="{left + plot_width / 2}" y="{height - 4}" text-anchor="middle" font-size="12">Sample after Context Switch</text>
...
def plot_branch_progress_metric_svg(rows, metric, ylabel, output_path):
  ...
  <rect x="{left}" y="{top}" width="{plot_width}" height="{plot_height}" fill="none" stroke="#333" />
  {"".join(lines)}
  <text x="{left + plot_width / 2}" y="{height - 10}" text-anchor="middle" font-size="14">Sample after Context Switch</text>
...
def plot_branch_progress_interactive(rows, output_path):
    ...
    for metric_idx, (_, ylabel) in enumerate(metrics):
        ...
        if metric_idx != plot_count - 1:
            ...
            layout[xaxis_name]["title"] = {"text": "Sample after Context Switch"}
        layout[yaxis_name] = {
