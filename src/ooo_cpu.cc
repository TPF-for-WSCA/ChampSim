...
#include "set.h"
#include "vmem.h"
#include "twig_profile.h"
#include "twig_prefetcher.h"
...
extern uint8_t knob_cloudsuite, pt, perfect_bp, perfect_btb, perfect_bpu;
extern uint8_t MAX_INSTR_DESTINATIONS;

extern string input_generalization;

extern bool generate_twig_trace;
extern bool use_twig_prefetcher;

extern VirtualMemory vmem;

TwigRecord twig_record;
TwigPrefetcher twig_prefetcher;

std::string O3_CPU::find_trace_short_name(std::string &full_path, O3_CPU::NameKind name_kind)
{
    fs::path p = full_path;
    if (!input_generalization.empty())
    {
        auto input_num = string(p.stem().c_str());
        auto app_name = string(p.parent_path().parent_path().filename().c_str());
        if (name_kind == O3_CPU::NameKind::TRACE_TRAIN)
            return app_name + "_" + input_num + "_" + input_generalization;
        else if (name_kind == O3_CPU::NameKind::TRACE)
            return app_name + "_trace_" + input_num;
        else
            return app_name + "_train_" + input_generalization;
    }
    if (pt)
    {
        return p.parent_path().filename().c_str();
    }
    else
    {
...
void O3_CPU::initialize_core()
{
    ...
    twig_record.init(short_name, generate_twig_trace);
    twig_prefetch_match = twig_prefetcher.init(short_name, use_twig_prefetcher);
}
...
uint32_t O3_CPU::init_instruction(ooo_model_instr arch_instr)
{
    ...
    if (instrs_to_read_this_cycle == 0)
        instrs_to_read_this_cycle = std::min((std::size_t)FETCH_WIDTH,
    ...
    if (!pt)
    {
        ...
        if (arch_instr.branch_type == BRANCH_RETURN || arch_instr.branch_type == BRANCH_INDIRECT ||
            arch_instr.branch_type == BRANCH_INDIRECT_CALL)
        {
            assert(arch_instr.branch_target != 0);
        }
    ...
    if (arch_instr.is_branch)
    {
        ...
        DP(if (warmup_complete[cpu]) {
            cout << "[BRANCH] instr_id: " << instr_unique_id << " ip: " << hex << arch_instr.ip << dec << " taken: "
                 << +arch_instr.branch_taken << endl;
        });

        num_branch++;

        uint64_t predict_latency = 0;
        ...
        {
            arch_instr.branch_mispredicted = predict_latency;
        }
        uint8_t branch_prediction = predict_branch(arch_instr.ip, predicted_branch_target, always_taken,
        ...
        else if (perfect_btb && branch_prediction == arch_instr.branch_taken)
        {
            predicted_branch_target = arch_instr.branch_target;
        }
        ...
        //        assert((arch_instr.branch_target != 0 && arch_instr.branch_taken == 1) ||
        //               (arch_instr.branch_target == 0 && arch_instr.branch_taken == 0));
        twig_record.lookup(
            arch_instr.instr_id,
            arch_instr.ip,
            arch_instr.branch_target,
        ...
        twig_prefetcher.prefetch(arch_instr.ip, arch_instr.branch_target, arch_instr.branch_type, this);
    }
...
void O3_CPU::fetch_instruction()
{
    ...
    for (auto it = IFETCH_BUFFER.begin(); it != end; ++it)
    {
        ...
        auto to_find = it->ip;
        auto way = std::find_if(dib_set.begin(), dib_set.end(), [to_find](dib_entry_t x)
                                { return x.valid && ((x.addr >> LOG2_DIB_WINDOW_SIZE) == (to_find >> LOG2_DIB_WINDOW_SIZE)); });
        if (way != dib_set.end())
        {
            ...
            std::for_each(dib_set.begin(), dib_set.end(), [hit_lru](dib_entry_t &x)
                          { if (x.lru <= hit_lru) x.lru++; });
            way->lru = 0;
        }
    }

    // TODO: Also need to search PREFETCH_BUFFER here. If hit on the oldest block, move it to CACHE.
    // scan through IFETCH_BUFFER to find instructions that need to be translated
    auto itlb_req_begin = std::find_if(IFETCH_BUFFER.begin(), IFETCH_BUFFER.end(),
                                       [](const ooo_model_instr &x)
                                       { return !x.translated; });
    uint64_t find_addr = itlb_req_begin->ip;
    auto itlb_req_end = std::find_if(itlb_req_begin, IFETCH_BUFFER.end(), [find_addr](const ooo_model_instr &x)
                                     { return (find_addr >> LOG2_PAGE_SIZE) != (x.ip >> LOG2_PAGE_SIZE); });
    ...
    auto l1i_req_end = std::find_if(l1i_req_begin, IFETCH_BUFFER.end(),
                                    ...
                                    {
                                        return (find_addr >> LOG2_BLOCK_SIZE) != (x.instruction_pa >> LOG2_BLOCK_SIZE);
                                    });
...
void O3_CPU::decode_instruction()
{
    ...
    while (available_decode_bandwidth > 0 && DECODE_BUFFER.has_ready() && !DISPATCH_BUFFER.full())
    {
        ...
        auto way = std::find_if(dib_set.begin(), dib_set.end(), [db_entry](dib_entry_t x)
                                { return x.valid && ((x.addr >> LOG2_DIB_WINDOW_SIZE) == (db_entry.ip >> LOG2_DIB_WINDOW_SIZE)); });

        // If we did not find the entry in the DIB, find a victim
        if (way == dib_set.end())
        {
            way = std::max_element(dib_set.begin(), dib_set.end(), [](dib_entry_t x, dib_entry_t y)
                                   { return !y.valid || (x.valid && x.lru < y.lru); }); // invalid ways compare LRU
        ...
        // TODO: Some branches are updated here
        if ((db_entry.branch_type == BRANCH_DIRECT_JUMP) || (db_entry.branch_type == BRANCH_DIRECT_CALL))
        {
            twig_record.update(
                db_entry.instr_id,
                db_entry.ip,
                db_entry.branch_target,
                db_entry.branch_type,
                current_core_cycle[cpu]);
        }

        // FDIP: Post Fetch Correction
        if ((db_entry.branch_type == BRANCH_DIRECT_JUMP) || (db_entry.branch_type == BRANCH_DIRECT_CALL))
        {
            l1i_prefetcher_resolved_branch_operate(db_entry, true);
        }
...
int O3_CPU::prefetch_code_line(uint64_t pf_v_addr)
{
    ...
    if (!L1I.PQ.full())
    {
        ...
        uint64_t pf_pa = (vmem.va_to_pa(cpu, pf_v_addr) & (~((1 << LOG2_PAGE_SIZE) - 1))) |
...
void O3_CPU::schedule_instruction()
{
    ...
    for (uint32_t i = ROB.head, count = 0; count < ROB.occupancy; i = (i + 1 == ROB.SIZE) ? 0 : i + 1, count++)
    {
        if ((ROB.entry[i].fetched != COMPLETED) || (ROB.entry[i].event_cycle > current_core_cycle[cpu]) ||
...
void O3_CPU::do_scheduling(uint32_t rob_index)
{
    ...
    else
    {
        ...
        if (ROB.entry[rob_index].reg_ready)
        {
            ...
            DP(if (warmup_complete[cpu]) {
                cout << "[ready_to_execute] " << __func__ << " instr_id: " << ROB.entry[rob_index].instr_id
                     << " rob_index: " << rob_index << " is added to ready_to_execute";
...
void O3_CPU::reg_dependency(uint32_t rob_index)
{
    ...
    DP(if (warmup_complete[cpu]) {
        for (uint32_t i = 0; i < NUM_INSTR_SOURCES; i++)
        {
            ...
            {
                cout << "[ROB] " << __func__ << " instr_id: " << ROB.entry[rob_index].instr_id << " is_memory: "
                     << +ROB.entry[rob_index].is_memory;
    ...
    for (uint32_t i = 0; i < MAX_INSTR_DESTINATIONS; i++)
    {
        ...
        if (ROB.entry[prior].destination_registers[i] == ROB.entry[current].source_registers[source_index])
        {
            ...
            ROB.entry[prior].registers_instrs_depend_on_me.insert(
                current); // this load cannot be executed until the prior store gets executed
            ...
            DP(if (warmup_complete[cpu]) {
                cout << "[ROB] " << __func__ << " instr_id: " << ROB.entry[current].instr_id << " is_memory: "
                     << +ROB.entry[current].is_memory;
...
void O3_CPU::do_memory_scheduling(uint32_t rob_index)
{
    ...
    if (not_available == 0)
    {
        ...
        DP(if (warmup_complete[cpu]) {
            cout << "[ROB] " << __func__ << " instr_id: " << ROB.entry[rob_index].instr_id << " rob_index: "
                 << rob_index;
        ...
        if (ROB.entry[rob_index].source_memory[i])
        {
            ...
            else
            {
                DP(if (warmup_complete[cpu]) {
                    ...
                    cout << " cannot be added in the load queue occupancy: " << LQ.occupancy << " cycle: "
        ...
        if (ROB.entry[rob_index].destination_memory[i])
        {
            ...
            else
            {
                DP(if (warmup_complete[cpu]) {
                    ...
                    cout << " cannot be added in the store queue occupancy: " << SQ.occupancy << " cycle: "
                         << current_core_cycle[cpu] << endl;
                });
            }
        }
...
void O3_CPU::add_load_queue(uint32_t rob_index, uint32_t data_index)
{
    ...
    for (uint32_t i = 0; i < SQ.SIZE; i++)
    {
        ...
        { // store-to-load forwarding check
            ...
            if ((LQ.entry[lq_index].producer_id == UINT64_MAX) &&
                (LQ.entry[lq_index].instr_id <= SQ.entry[i].instr_id))
            { // WAR
                ...
                DP(if (warmup_complete[cpu]) {
                    cout << "[LQ] " << __func__ << " instr_id: " << LQ.entry[lq_index].instr_id << " reset fetched: "
                         << +LQ.entry[lq_index].fetched;
                    cout << " to obey WAR store instr_id: " << SQ.entry[i].instr_id << " cycle: "
                         << current_core_cycle[cpu] << endl;
                });
            }
        }
    ...
    { // we have a store-to-load forwarding
            ...
            (SQ.entry[forwarding_index].event_cycle <= current_core_cycle[cpu]))
        {
            LQ.entry[lq_index].physical_address =
                (SQ.entry[forwarding_index].physical_address & ~(uint64_t)((1 << LOG2_BLOCK_SIZE) - 1)) |
            ...
            DP(if (warmup_complete[cpu]) {
                ...
                cout << SQ.entry[forwarding_index].instr_id << " remain_num_ops: "
                     << ROB.entry[fwr_rob_index].num_mem_ops << " cycle: " << current_core_cycle[cpu] << endl;
            });
    ...
    if (LQ.entry[lq_index].virtual_address &&
        (LQ.entry[lq_index].producer_id == UINT64_MAX))
    { // not released and no forwarding
        ...
        DP(if (warmup_complete[cpu]) {
            cout << "[RTL0] " << __func__ << " instr_id: " << LQ.entry[lq_index].instr_id << " rob_index: "
                 << LQ.entry[lq_index].rob_index << " is added to RTL0";
    ...
    DP(if (warmup_complete[cpu]) {
        ...
        cout << " is added in the LQ address: " << hex << LQ.entry[lq_index].virtual_address << dec << " translated: "
             << +LQ.entry[lq_index].translated;
        cout << " fetched: " << +LQ.entry[lq_index].fetched << " index: " << lq_index << " occupancy: " << LQ.occupancy
...
void O3_CPU::mem_RAW_dependency(uint32_t prior, uint32_t current, uint32_t data_index, uint32_t lq_index)
{
    for (uint32_t i = 0; i < MAX_INSTR_DESTINATIONS; i++)
    {
        ...
        if (ROB.entry[prior].destination_memory[i] ==
            ROB.entry[current].source_memory[data_index])
        { //  store-to-load forwarding check
            ...
            DP(if (warmup_complete[cpu]) {
                cout << "[LQ] " << __func__ << " RAW producer instr_id: " << ROB.entry[prior].instr_id
                     << " consumer_id: " << ROB.entry[current].instr_id << " lq_index: " << lq_index;
...
void O3_CPU::add_store_queue(uint32_t rob_index, uint32_t data_index)
{
    ...
    */
    ...
    DP(if (warmup_complete[cpu]) {
        ...
        cout << " is added in the SQ translated: " << +SQ.entry[sq_index].translated << " fetched: "
             << +SQ.entry[sq_index].fetched << " is_producer: " << +ROB.entry[rob_index].is_producer;
    ...
    while (store_issued < SQ_WIDTH)
    {
        if (RTS0[RTS0_head] < SQ_SIZE)
        {
            ...
            if (SQ.entry[sq_index].event_cycle <= current_core_cycle[cpu])
            {
                ...
                if (knob_cloudsuite)
                    data_packet.address =
                        ((SQ.entry[sq_index].virtual_address >> LOG2_PAGE_SIZE) << 9) | SQ.entry[sq_index].asid[1];
                else
                ...
                DP(if (warmup_complete[cpu]) {
                    cout << "[RTS0] " << __func__ << " instr_id: " << SQ.entry[sq_index].instr_id << " rob_index: "
                         << SQ.entry[sq_index].rob_index << " is popped from to RTS0";
    ...
    while (load_issued < LQ_WIDTH)
    {
        if (RTL0[RTL0_head] < LQ_SIZE)
        {
            ...
            if (LQ.entry[lq_index].event_cycle <= current_core_cycle[cpu])
            {
                ...
                if (knob_cloudsuite)
                    data_packet.address =
                        ((LQ.entry[lq_index].virtual_address >> LOG2_PAGE_SIZE) << 9) | LQ.entry[lq_index].asid[1];
                else
                ...
                DP(if (warmup_complete[cpu]) {
                    cout << "[RTL0] " << __func__ << " instr_id: " << LQ.entry[lq_index].instr_id << " rob_index: "
                         << LQ.entry[lq_index].rob_index << " is popped to RTL0";
                    cout << " head: " << RTL0_head << " tail: " << RTL0_tail << endl;
                });
...
void O3_CPU::execute_store(uint32_t rob_index, uint32_t sq_index, uint32_t data_index)
{
    ...
    DP(if (warmup_complete[cpu]) {
        ...
        cout << " full_address: " << SQ.entry[sq_index].physical_address << dec << " remain_mem_ops: "
             << ROB.entry[rob_index].num_mem_ops;
            ...
            { // which one is dependent?
                if (ROB.entry[dependent].source_memory[j] && ROB.entry[dependent].source_added[j])
                {
                    ...
                    { // this is required since a single instruction can issue multiple loads
                        ...
                        {
                            cerr << "[SQ2] " << __func__ << " lq_index: " << lq_index << " producer_id: "
                        ...
                        LQ.entry[lq_index].physical_address =
                            (SQ.entry[sq_index].physical_address & ~(uint64_t)((1 << LOG2_BLOCK_SIZE) - 1)) |
                            (LQ.entry[lq_index].virtual_address & ((1 << LOG2_BLOCK_SIZE) - 1));
                        LQ.entry[lq_index].translated = COMPLETED;
                        LQ.entry[lq_index].fetched = COMPLETED;
                        ...
                        DP(if (warmup_complete[cpu]) {
                            ...
                            cout << SQ.entry[sq_index].instr_id << " remain_num_ops: "
                                 << ROB.entry[fwr_rob_index].num_mem_ops << " cycle: " << current_core_cycle[cpu]
                                 << endl;
                        });
...
uint32_t O3_CPU::complete_execution(uint32_t rob_index)
{
    ...
    if (ROB.entry[rob_index].is_memory == 0)
    {
            ...
            (ROB.entry[rob_index].event_cycle <= current_core_cycle[cpu]))
        {
            ...
            if (ROB.entry[rob_index].branch_type == BRANCH_CONDITIONAL)
            {
                twig_record.update(
            ...
            if (ROB.entry[rob_index].is_branch)
            {
                l1i_prefetcher_resolved_branch_operate(ROB.entry[rob_index], false);
            }
            ...
            DP(if (warmup_complete[cpu]) {
                ...
                cout << " branch_mispredicted: " << +ROB.entry[rob_index].branch_mispredicted << " fetch_stall: "
                     << +fetch_stall;
    ...
    else
    {
        if (ROB.entry[rob_index].num_mem_ops == 0)
        {
                ...
                (ROB.entry[rob_index].event_cycle <= current_core_cycle[cpu]))
            {
                ...
                DP(if (warmup_complete[cpu]) {
                    cout << "[ROB] " << __func__ << " instr_id: " << ROB.entry[rob_index].instr_id;
                    cout << " is_memory: " << +ROB.entry[rob_index].is_memory << " branch_mispredicted: "
                         << +ROB.entry[rob_index].branch_mispredicted;
                    cout << " fetch_stall: " << +fetch_stall << " event: " << ROB.entry[rob_index].event_cycle
            ...
            if (ROB.entry[rob_index].registers_index_depend_on_me[j].search(i))
            {
                ...
                if (ROB.entry[i].num_reg_dependent == 0)
                {
                    ...
                    else
                    {
                        ...
                        DP(if (warmup_complete[cpu]) {
                            cout << "[ready_to_execute] " << __func__ << " instr_id: " << ROB.entry[i].instr_id
                                 << " rob_index: " << i << " is added to ready_to_execute";
                            cout << " head: " << ready_to_execute_head << " tail: " << ready_to_execute_tail << endl;
                        });
                ...
                DP(if (warmup_complete[cpu]) {
                    cout << "[ROB] " << __func__ << " instr_id: " << ROB.entry[rob_index].instr_id
                         << " releases instr_id: ";
                    cout << ROB.entry[i].instr_id << " reg_index: " << +ROB.entry[i].source_registers[j]
                         << " num_reg_dependent: " << ROB.entry[i].num_reg_dependent << " cycle: "
                         << current_core_cycle[cpu] << endl;
                });
            }
        }
...
void O3_CPU::handle_memory_return()
{
    ...
    while (available_fetch_bandwidth > 0 && to_read > 0 && !ITLB_bus.PROCESSED.empty() &&
           ITLB_bus.PROCESSED.front().event_cycle <= current_core_cycle[cpu])
    {
    ...
    while (available_fetch_bandwidth > 0 && to_read > 0 && !L1I_bus.PROCESSED.empty() &&
           L1I_bus.PROCESSED.front().event_cycle <= current_core_cycle[cpu])
    {
        ...
        while (!l1i_entry.instr_depend_on_me.empty())
        {
            ...
            if (available_fetch_bandwidth > 0)
            {
                if ((it->instruction_pa >> LOG2_BLOCK_SIZE) == (l1i_entry.address) && it->fetched != 0 &&
                    it->translated == COMPLETED)
                {
                    it->fetched = COMPLETED;
                    available_fetch_bandwidth--;
    ...
    while (to_read > 0 && !DTLB_bus.PROCESSED.empty() &&
           (DTLB_bus.PROCESSED.front().event_cycle <= current_core_cycle[cpu]))
    { // DTLB
        PACKET &dtlb_entry = DTLB_bus.PROCESSED.front();

        for (auto sq_merged : dtlb_entry.sq_index_depend_on_me)
        {
            SQ.entry[sq_merged].physical_address = (dtlb_entry.data << LOG2_PAGE_SIZE) |
                                                   (SQ.entry[sq_merged].virtual_address &
        ...
        {
            LQ.entry[lq_merged].physical_address = (dtlb_entry.data << LOG2_PAGE_SIZE) |
                                                   (LQ.entry[lq_merged].virtual_address &
                                                    ((1 << LOG2_PAGE_SIZE) - 1)); // translated address
    ...
    while (to_read > 0 && !L1D_bus.PROCESSED.empty() &&
           (L1D_bus.PROCESSED.front().event_cycle <= current_core_cycle[cpu]))
    { // L1D
...
void O3_CPU::release_load_queue(uint32_t lq_index)
{
    // release LQ entries
    DP(if (warmup_complete[cpu]) {
        cout << "[LQ] " << __func__ << " instr_id: " << LQ.entry[lq_index].instr_id << " releases lq_index: "
             << lq_index;
...
void O3_CPU::retire_rob()
{
    ...
    for (uint32_t n = 0; n < RETIRE_WIDTH; n++)
    {
        ...
        {
            DP(if (warmup_complete[cpu]) {
                cout << "[ROB] " << __func__ << " instr_id: " << ROB.entry[ROB.head].instr_id << " head: " << ROB.head
                     << " is not executed yet" << endl;
            });
            return;
        }
        ...
        for (uint32_t i = 0; i < MAX_INSTR_DESTINATIONS; i++)
        {
            if (ROB.entry[ROB.head].sq_index[i] != UINT32_MAX)
            {
                ...
                DP(if (warmup_complete[cpu]) {
                    cout << "[SQ] " << __func__ << " instr_id: " << ROB.entry[ROB.head].instr_id
