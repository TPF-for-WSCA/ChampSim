...
long O3_CPU::operate()
{
  ...
  if (false && show_heartbeat && current_cycle >= next_print_inst_cycle) {
    fmt::print("HEARTBEET : cycles: {} last_instr: {} oldest_instr: {} read: {} wp_read: {} fetched_cnt: {} dib: {} promoted: {} decoded: {} dispatched: {} "
               "executed: {} retired: {}\n",
               current_cycle, IFETCH_BUFFER.back().instr_id, IFETCH_BUFFER.front().instr_id, read, wp_read, fetched_cnt, dib, promoted, decoded, dispatched,
               executed, num_retired);
...
bool O3_CPU::add_wrongpath_instruction()
{
  ...
  auto pred = impl_btb_prediction(prev_wrong_ip);
...
long O3_CPU::check_dib()
{
  ...
  auto [wrongpath_window_begin, wrongpath_window_end] = champsim::get_span(begin, std::end(IFETCH_BUFFER_WRONGPATH), dec_cnt);
...
long O3_CPU::fetch_instruction()
{
  ...
  { // WRONGPATH BLOCK
    ...
    for (; to_read > 0 && l1i_req_begin_wp != std::end(IFETCH_BUFFER_WRONGPATH); --to_read) {
      auto l1i_req_end_wp = std::adjacent_find(l1i_req_begin_wp, std::end(IFETCH_BUFFER_WRONGPATH), no_match_ip);
      if (l1i_req_end_wp != std::end(IFETCH_BUFFER_WRONGPATH))
        l1i_req_end_wp = std::next(l1i_req_end_wp); // adjacent_find returns the first of the non-equal elements
...
long O3_CPU::dispatch_instruction()
{
  ...
  while (available_dispatch_bandwidth > 0 && !std::empty(DISPATCH_BUFFER) && DISPATCH_BUFFER.front().event_cycle < current_cycle && std::size(ROB) != ROB_SIZE
    ...
    if (DISPATCH_BUFFER.front().wrongpath) {
      std::cout << "WARNING: WRONGPATH REACHED DISPATCH" << std::endl;
  ...
  for (auto l1i_bw = FETCH_WIDTH, to_read = L1I_BANDWIDTH; l1i_bw > 0 && to_read > 0 && !L1I_bus.lower_level->returned.empty(); --to_read) {
    ...
    while (l1i_bw > 0 && !l1i_entry.instr_depend_on_me.empty()) {
      auto fetched = std::find_if(std::begin(IFETCH_BUFFER), std::end(IFETCH_BUFFER),
                                  [l1i_entry](const ooo_model_instr& x) { return l1i_entry.instr_depend_on_me.front() == x.instr_id; });
