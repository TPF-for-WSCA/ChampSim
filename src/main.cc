...
uint8_t warmup_complete[NUM_CPUS] = {}, simulation_complete[NUM_CPUS] = {}, trace_ended[NUM_CPUS] = {}, all_warmup_complete = 0, all_simulation_complete = 0,
        ...
        knob_stop_at_completion = 1, knob_collect_offsets = 0, knob_min_50M = 0;
...
void write_offsets(O3_CPU* cpu, int cpu_id)
{
  ...
  string filename = "cpu" + std::to_string(cpu_id) + "_num_bits_count.tsv";
  auto csv_file_path = csv_result_path / filename;
  csv_file.open(csv_file_path, std::ios::out);
  if (!csv_file) {
    std::cerr << "COULD NOT CREATE/OPEN FILE " << csv_file_path << std::endl;
  ...
  num_files_required =
      std::min(num_files_required,
               cpu->BTB_WAYS); // This is a sanity check to prevent a bug from surfacing (BTB_NON_INDIRECT is not guaranteed to be initialized properly)
