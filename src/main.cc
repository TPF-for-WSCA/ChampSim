...
uint8_t warmup_complete[NUM_CPUS],
    ...
    predecode_latency = 0,
    total_btb_ways = 4;
//        use_default_btb_record = 0;
uint64_t total_btb_entries = 8192;

uint8_t train_total_btb_ways = 4;
uint64_t train_total_btb_entries = 8192;

string input_generalization = "";

bool generate_twig_trace = false;
bool use_twig_prefetcher = false;
...
void print_branch_stats()
{
    for (uint32_t i = 0; i < NUM_CPUS; i++)
    {
        ...
        */
        ...
        cout << "BRANCH_RETURN: " << (1000.0 * ooo_cpu[i].branch_type_misses[6] / (ooo_cpu[i].num_retired - ooo_cpu[i].begin_sim_instr)) << endl
             << endl;
...
int main(int argc, char **argv)
{
    ...
    while (1)
    {
        static struct option long_options[] =
            {
                ...
                {"pt", no_argument, 0, 'p'},
                {"high_bandwidth", no_argument, 0, 'b'},
                {"traces", no_argument, 0, 't'},
                {"perfect", required_argument, 0, 'u'},
                {"predecode_latency", required_argument, 0, 'l'},
                {"total_btb_ways", required_argument, 0, 'a'},
                {"total_btb_entries", required_argument, 0, 'e'},
                {"train_total_btb_ways", required_argument, 0, 'y'},
                {"train_total_btb_entries", required_argument, 0, 'x'},
                {"input_generalization", required_argument, 0, 'g'},
                {"twig", no_argument, 0, '0'},
                {"twig_prefetch", no_argument, 0, '1'},
                //            {"use_default_btb_record", no_argument, 0, 'd'},
                {0, 0, 0, 0}};
        ...
        case 'u':
            ...
            else if (strcmp(optarg, "btb") == 0)
            {
                perfect_btb = 1;
            }
            else if (strcmp(optarg, "bpu") == 0)
            {
                perfect_bpu = 1;
            }
            break;
        case 'l':
            predecode_latency = atoi(optarg);
            break;
        case 'a':
            total_btb_ways = atoi(optarg);
            break;
        case 'e':
            total_btb_entries = atoi(optarg);
            if (total_btb_entries < 1024)
                total_btb_entries *= 1024;
            cout << "total_btb_entries " << total_btb_entries << endl;
            break;
        case 'y':
            train_total_btb_ways = atoi(optarg);
            break;
        case 'x':
            train_total_btb_entries = atoi(optarg);
            if (train_total_btb_entries < 1024)
                train_total_btb_entries *= 1024;
            cout << "train_total_btb_entries " << train_total_btb_entries << endl;
            break;
        case 'g':
            input_generalization = optarg;
            break;
            //            case 'd':
            //                use_default_btb_record = 1;
            //                break;
        case '0':
            generate_twig_trace = true;
            break;
        case '1':
            use_twig_prefetcher = true;
            break;
        default:
            abort();
        }
    ...
    for (int i = 0; i < argc; i++)
    {
        if (found_traces)
        {
            ...
            trace_strings.emplace_back(argv[i]);
    ...
    for (int i = 0; i < NUM_CPUS; i++)
    {
        ...
        ooo_cpu.at(i).trace_name = trace_strings[i];
        ooo_cpu.at(i).initialize_btb();
        ooo_cpu.at(i).initialize_core();
        ooo_cpu.at(i).get_program_name(argv[0]);
