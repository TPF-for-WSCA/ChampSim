...
public:
  ...
  uint64_t instr_unique_id = 0, completed_executions = 0, begin_sim_cycle = 0, begin_sim_instr = 0, last_sim_cycle = 0, last_sim_instr = 0,
           ...
           next_print_instruction = STAT_PRINTING_PERIOD, num_retired = 0, frontend_stall_cycles = 0, indirect_branches = 0;
...
#include "ooo_cpu_modules.inc"
  ...
  O3_CPU(uint32_t cpu, double freq_scale, std::size_t dib_set, std::size_t dib_way, std::size_t dib_window, std::size_t ifetch_buffer_size,
         ...
         bpred_t bpred_type, btb_t btb_type, size_t btb_sets, size_t btb_ways, ipref_t ipref_type)
      : champsim::operable(freq_scale), cpu(cpu), dib_set(dib_set), dib_way(dib_way), dib_window(dib_window), IFETCH_BUFFER(ifetch_buffer_size * 2),
        ...
        bpred_type(bpred_type), btb_type(btb_type), BTB_SETS(btb_sets), BTB_WAYS(btb_ways), ipref_type(ipref_type)
  {
