...
uint32_t CACHE::prefetcher_cache_operate(uint64_t address, uint64_t ip, uint8_t cache_hit, bool useful_prefetch, uint8_t type, uint32_t metadata_in)
{
  ...
  update_global_state(ip, page, offset, address);
  /* per page state tracking */
  Scooby_STEntry* stentry = update_local_state(ip, page, offset, address);

  /* Measure state.
   * state can contain per page local information like delta signature, ip signature etc.
...
void update_global_state(uint64_t pc, uint64_t page, uint32_t offset, uint64_t address) { /* @rbera TODO: implement */ }
...
//----------------------------------------------------//
uint32_t predict(uint64_t base_address, uint64_t page, uint32_t offset, State* state, std::vector<uint64_t>& pref_addr)
{
...
//----------------------------------------------------//
uint32_t get_dyn_pref_degree(float max_to_avg_q_ratio, uint64_t page, int32_t action)
{
  uint32_t counted = false;
  uint32_t degree = 1;
  bool high_bw = false;  // TODO: FIX BW WITH UPSTREAM
...
{
  bool high_bw = PYTHIA::enable_hbw_reward;
  int32_t reward = 0;
...
std::vector<Scooby_PTEntry*> search_pt(uint64_t address, bool search_all)
{
...
bool is_high_bw(uint8_t bw_level) { return bw_level >= PYTHIA::high_bw_thresh ? true : false; }