...
void l1i_add_bb_size_hist_table(uint64_t line_addr, uint32_t bb_size, bool wrong_path)
{
  ...
  if (wrong_path && ((bb_size & L1I_MERGE_BBSIZE_MAX_VALUE) > l1i_hist_table[l1i_cpu_id][index].bb_size))
    l1i_hist_table[l1i_cpu_id][index].wrong_path = true;
...
uint64_t l1i_get_src_entangled_hist_table(uint64_t line_addr, uint32_t pos_hist, uint64_t latency, bool& wrong_path, uint32_t skip = 0)
{
  ...
  for (uint32_t count = 0, i = next_pos; i != first; count++, i = (i + L1I_HIST_TABLE_MASK) % L1I_HIST_TABLE_ENTRIES) {
    ...
    if (l1i_hist_table[l1i_cpu_id][i].tag && time_i >= latency) {
      num_searched++;
      // cout << (num_searched - 1) << " Format " << l1i_hist_table[l1i_cpu_id][i].format << " dsts " << l1i_hist_table[l1i_cpu_id][i].num_dst << " new " <<
      // l1i_get_format_entangled(l1i_hist_table[l1i_cpu_id][i].tag, tag) << endl;
      uint32_t format_i = l1i_get_format_entangled(l1i_hist_table[l1i_cpu_id][i].tag, tag);
      uint32_t new_format =
          (l1i_hist_table[l1i_cpu_id][i].num_dst == 0 || format_i < l1i_hist_table[l1i_cpu_id][i].format) ? format_i : l1i_hist_table[l1i_cpu_id][i].format;
      if (num_searched == 1) {
  ...
  if (best_entangled) {
    ...
    if (l1i_hist_table[l1i_cpu_id][pos_best].wrong_path || l1i_hist_table[l1i_cpu_id][pos_hist].wrong_path)
      wrong_path = true;
...
void l1i_print_hist_table()
{
  ...
  for (uint32_t count = 0; count < L1I_HIST_TABLE_ENTRIES; count++) {
    // Against the time overflow
    std::cout << std::hex << l1i_hist_table[l1i_cpu_id][i].tag << std::dec << " " << l1i_hist_table[l1i_cpu_id][i].bb_size << " "
              << l1i_hist_table[l1i_cpu_id][i].instr_id << " " << l1i_hist_table[l1i_cpu_id][i].wrong_path << " | ";
...
bool l1i_different_timing_entry_src(uint64_t line_addr, uint32_t source_set, uint32_t source_way)
{
  uint32_t index = l1i_find_timing_mshr_entry(line_addr);
  if (index < L1I_TIMING_MSHR_SIZE)
    if (l1i_timing_mshr_table[l1i_cpu_id][index].source_set != source_set || l1i_timing_mshr_table[l1i_cpu_id][index].source_way != source_way)
      return true;
  uint32_t set = line_addr % L1I_SET;
  uint32_t way = l1i_find_timing_cache_entry(line_addr);
  if (way < L1I_WAY)
    if (l1i_timing_cache_table[l1i_cpu_id][set][way].source_set != source_set || l1i_timing_cache_table[l1i_cpu_id][set][way].source_way != source_way)
      return true;
  return false;
}
...
void l1i_move_history_head_to_entangled()
{
  ...
  if (l1i_hist_table[l1i_cpu_id][l1i_hist_table_head[l1i_cpu_id]].bb_size) {
    ...
    l1i_add_bbsize_table(l1i_hist_table[l1i_cpu_id][l1i_hist_table_head[l1i_cpu_id]].tag,
                         std::max(max_bb_size, l1i_hist_table[l1i_cpu_id][l1i_hist_table_head[l1i_cpu_id]].bb_size));
    ...
    if (l1i_hist_table[l1i_cpu_id][l1i_hist_table_head[l1i_cpu_id]].wrong_path)
      l1i_stats_recorded_wrong_path_info++;
  }

  // move entangling pair when exiting the history
  if (l1i_hist_table[l1i_cpu_id][l1i_hist_table_head[l1i_cpu_id]].ent_src) {
    l1i_add_entangled_table(l1i_hist_table[l1i_cpu_id][l1i_hist_table_head[l1i_cpu_id]].ent_src,
                            l1i_hist_table[l1i_cpu_id][l1i_hist_table_head[l1i_cpu_id]].tag);
    l1i_stats_recorded_info++;
    // if (l1i_hist_table[l1i_cpu_id][l1i_hist_table_head[l1i_cpu_id]].wrong_path) { l1i_print_hist_table(); assert(false); }
    if (l1i_hist_table[l1i_cpu_id][l1i_hist_table_head[l1i_cpu_id]].wrong_path)
      l1i_stats_recorded_wrong_path_info++;
...
uint32_t O3_CPU::prefetcher_cache_operate(uint64_t addr, uint8_t cache_hit, uint8_t prefetch_hit, uint32_t metadata_in)
{
  ...
  if (!cache_hit)
    assert(!prefetch_hit);
  if (!cache_hit)
    assert(l1i_find_timing_cache_entry(line_addr) == L1I_WAY);
  if (cache_hit)
    assert(l1i_find_timing_cache_entry(line_addr) < L1I_WAY);
  ...
  for (uint32_t i = 1; i <= bb_size; i++) {
    ...
    if (!l1i_ongoing_request(pf_addr >> LOG2_BLOCK_SIZE)) {
      if (static_cast<CACHE*>(L1I_bus.lower_level)->prefetch_line(pf_addr, true, 0)) {
    ...
    if (entangled_line_addr && (entangled_line_addr != line_addr)) {
      ...
      for (uint32_t i = 0; i <= bb_size; i++) {
        ...
        if (!l1i_ongoing_request(pf_line_addr)) {
          if (static_cast<CACHE*>(L1I_bus.lower_level)->prefetch_line(pf_line_addr << LOG2_BLOCK_SIZE, true, 0)) {
  ...
  if (!consecutive && l1i_basic_block_merge_diff == 0) {
    if ((l1i_find_hist_entry(line_addr) == L1I_HIST_TABLE_ENTRIES)) {
      pos_hist = l1i_add_hist_table(line_addr, 0, format, num_entangled, metadata_in);
    } else {
...
uint32_t O3_CPU::prefetcher_cache_fill(uint64_t addr, uint32_t set, uint32_t way, uint8_t prefetch, uint64_t evicted_addr, uint32_t metadata_in)
{
  ...
  if (latency && pos_hist < L1I_HIST_TABLE_ENTRIES) {
    ...
    if (src_entangled) {
      ...
      if (wrong_path)
        l1i_hist_table[l1i_cpu_id][pos_hist].wrong_path = true;
    }
  }
